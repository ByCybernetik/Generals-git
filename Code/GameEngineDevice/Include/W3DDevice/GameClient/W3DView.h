...
typedef struct 
{
	...
	Real		curFrame;							///< Current frame (Real: wall-clock fractional steps on Linux).
	Real		angle;
	Real		startZoom;
	Real		endZoom;
	Real		startPitch;
...
private:
	...
	void rotateCameraOneFrame(Real frameDelta = 1.0f);	///< Advance rotate camera (1 = one logic frame).
	void zoomCameraOneFrame(Real frameDelta = 1.0f);		///< Advance zoom camera.
	void pitchCameraOneFrame(Real frameDelta = 1.0f);		///< Advance pitch camera.
