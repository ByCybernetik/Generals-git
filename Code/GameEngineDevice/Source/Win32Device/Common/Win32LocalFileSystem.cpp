...
Bool Win32LocalFileSystem::doesFileExist(const Char *filename) const
{
	...
	if (filename != nullptr) {
		...
		size_t j = 0;
		for (size_t i = 0; filename[i] != '\0' && j + 1 < _MAX_PATH; ++i) {
			normalized[j++] = (filename[i] == '\\') ? '/' : filename[i];
		}
		normalized[j] = '\0';
		if (_access(normalized, 0) == 0) {
			return TRUE;
		}
		char resolved[_MAX_PATH];
		if (Platform_Resolve_File_Path_Case(normalized, resolved, sizeof(resolved)) &&
			_access(resolved, 0) == 0) {
			return TRUE;
		}
	}
#endif
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
				RenegadeNormalizeAsciiPath(newFilename);
#endif
...
Bool Win32LocalFileSystem::getFileInfo(const AsciiString& filename, FileInfo *fileInfo) const 
{
	...
	findHandle = FindFirstFile(path.str(), &findData);
#else
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		RenegadeNormalizeAsciiPath(directory);
#endif
