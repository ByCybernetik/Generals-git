...
#if defined(RENEGADE_VULKAN)
static void Sync_Radar_Texture(TextureClass *texture, SurfaceClass *surface)
{
	if (texture != nullptr && surface != nullptr) {
		ww3d_vulkan::Sync_Procedural_Texture_From_Surface(texture, surface);
	}
}
#else
static void Sync_Radar_Texture(TextureClass *, SurfaceClass *)
{
}
#endif
...
//-------------------------------------------------------------------------------------------------
void W3DRadar::renderObjectList(
	const RadarObject *listHead,
	TextureClass *texture,
	Bool calcHero,
	SurfaceClass *surfaceIn)
{
	...
	const Bool ownSurface = (surfaceIn == NULL);
	SurfaceClass *surface = surfaceIn;
	if( ownSurface )
	...
	if( ownSurface )
	{
		Sync_Radar_Texture(texture, surface);
...
void W3DRadar::reset( void )
{
	...
	{
		surface->Clear();
		Sync_Radar_Texture(m_terrainTexture, surface);
		REF_PTR_RELEASE(surface);
	}
	...
	{
		surface->Clear();
		Sync_Radar_Texture(m_overlayTexture, surface);
		REF_PTR_RELEASE(surface);
	}
...
void W3DRadar::clearShroud()
{
	...
	Sync_Radar_Texture(m_shroudTexture, surface);
	REF_PTR_RELEASE(surface);
}
...
void W3DRadar::draw( Int pixelX, Int pixelY, Int width, Int height )
{
	...
	if( TheGameClient->getFrame() % OVERLAY_REFRESH_RATE == 0 )
	{
		...
		// rebuild the object overlay on the same CPU surface
		renderObjectList( getObjectList(), m_overlayTexture, FALSE, surface );
		renderObjectList( getLocalObjectList(), m_overlayTexture, TRUE, surface );
		Sync_Radar_Texture(m_overlayTexture, surface);
		REF_PTR_RELEASE(surface);
