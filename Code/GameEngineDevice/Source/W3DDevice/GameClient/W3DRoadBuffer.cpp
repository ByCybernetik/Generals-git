...
void RoadType::loadTexture(AsciiString path, Int ID)
{
	...
	/// Topology-only / no WW3D device (e.g. Linux WorldBuilder bake path): skip DX textures & buffers.
	WW3DAssetManager *pMgr = W3DAssetManager::Get_Instance();
	if (pMgr == NULL || !DX8Wrapper::Is_Initted()) {
		m_roadTexture = NULL;
		m_vertexRoad = NULL;
		m_indexRoad = NULL;
		return;
															...
															Int minX, Int maxX, Int minY, Int maxY, RefRenderObjListIterator *pDynamicLightsIterator)
{
	...
	/*
	 * Shroud/FOW for roads is applied in the Vulkan base road pass
	 * (FLAG_MESH_INLINE_SHROUD). A separate multiplicative overlay stacks
	 * darkening wherever tee/curve segments overlap.
	 */
#if defined(RENEGADE_VULKAN)
	if (!(DX8Wrapper::Vulkan_Device_Active()))
#endif
	if (!wireframe && TheTerrainRenderObject && TheTerrainRenderObject->getShroud() &&
			TheTerrainRenderObject->getShroud()->getShroudTexture())
	{
		...
		W3DShaderManager::setTexture(0, shroudTex);
		...
		for (stacking = 0; stacking <= maxStacking; stacking++) {
			for (i = 0; i < m_maxRoadTypes; i++) {
				...
				if (m_roadTypes[i].getNumIndices() == 0) {
					continue;
				}
				DX8Wrapper::Set_Texture(0, m_roadTypes[i].getRoadTexture());
				DX8Wrapper::Set_Texture(1, shroudTex);
				DX8Wrapper::Set_Index_Buffer(m_roadTypes[i].getIB(), 0);
				DX8Wrapper::Set_Vertex_Buffer(m_roadTypes[i].getVB());
