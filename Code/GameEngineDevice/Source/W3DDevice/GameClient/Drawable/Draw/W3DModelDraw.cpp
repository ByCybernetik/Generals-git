...
#include "GameLogic/WeaponSlotTypeNameTables.h"
...
void W3DModelDraw::adjustAnimation(const ModelConditionInfo* prevState, Real prevAnimFraction)
{
	...
	if (numAnims > 0)
	{
		...
		if (m_renderObject && animHandle)
		{
			...
			/* State change owns the HLOD mode; clear pause so death ONCE anims aren't stuck MANUAL. */
			m_pauseAnimation = false;
			m_animationMode = m_curState->m_mode;
...
void W3DModelDraw::setPauseAnimation(Bool pauseAnim)
{
	...
	{
		/*
		 * Flag can say "playing" while HLOD was left in ANIM_MODE_MANUAL by
		 * validateCachedBones / a failed unpause. Repair so death anims run.
		 */
		if (!pauseAnim && m_renderObject &&
				m_renderObject->Class_ID() == RenderObjClass::CLASSID_HLOD)
		{
			float framenum, dummy;
			int mode, numFrames;
			HLodClass* hlod = (HLodClass*)m_renderObject;
			HAnimClass* anim = hlod->Peek_Animation_And_Info(framenum, numFrames, mode, dummy);
			if (anim && mode == RenderObjClass::ANIM_MODE_MANUAL)
			{
				int restore = m_animationMode;
				if (restore == RenderObjClass::ANIM_MODE_MANUAL)
				{
					restore = (m_curState != NULL)
						? m_curState->m_mode
						: RenderObjClass::ANIM_MODE_LOOP;
				}
				m_animationMode = restore;
				hlod->Set_Animation(anim, framenum, restore);
			}
		}
		return;
	}
...
void W3DModelDrawModuleData::xfer( Xfer *x )
{
	...
	for (ModelConditionVector::iterator it = m_conditionStates.begin(); it != m_conditionStates.end(); ++it)
	{
		...
		if (info->m_validStuff)
		{
			...
			{
				for (ModelConditionInfo::WeaponBarrelInfoVec::iterator wit = info->m_weaponBarrelInfoVec[j].begin(); wit != info->m_weaponBarrelInfoVec[j].end(); ++wit)
				{
