...
void drawSkinnyBorder( Int x, Int y, Int width, Int height)
{
	...
	Int size = 5;
	Int halfSize = size / 2;
	static const Image *s_frameT = NULL;
	static const Image *s_frameB = NULL;
	static const Image *s_frameL = NULL;
	static const Image *s_frameR = NULL;
	static const Image *s_cornerUL = NULL;
	static const Image *s_cornerUR = NULL;
	static const Image *s_cornerLL = NULL;
	static const Image *s_cornerLR = NULL;
	if (s_frameT == NULL && TheMappedImageCollection != NULL) {
		s_frameT = TheMappedImageCollection->findImageByName("FrameT");
		s_frameB = TheMappedImageCollection->findImageByName("FrameB");
		s_frameL = TheMappedImageCollection->findImageByName("FrameL");
		s_frameR = TheMappedImageCollection->findImageByName("FrameR");
		s_cornerUL = TheMappedImageCollection->findImageByName("FrameCornerUL");
		s_cornerUR = TheMappedImageCollection->findImageByName("FrameCornerUR");
		s_cornerLL = TheMappedImageCollection->findImageByName("FrameCornerLL");
		s_cornerLR = TheMappedImageCollection->findImageByName("FrameCornerLR");
	}
	const Image *image1 = s_frameT;
	const Image *image2 = s_frameB;
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/*
	 * One stretched draw per edge instead of ~5px tiles (~100–140 drawImage/frame
	 * on the Skirmish map preview). Matches blitBorderRect Linux path.
	 */
	LinuxUIDrawImageRepeatedH(
		TheDisplay, image1, originalX + 3, y, maxX - 5, y + size);
	LinuxUIDrawImageRepeatedH(
		TheDisplay, image2, originalX + 3, y2, maxX - 5, y2 + size);
#else
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	LinuxUIDrawImageRepeatedH(
		TheDisplay, image1, x, originalY + 3, x + size, maxY - 5);
	LinuxUIDrawImageRepeatedH(
		TheDisplay, image2, x2, originalY + 3, x2 + size, maxY - 5);
#else
	...
	TheDisplay->drawImage( s_cornerUL,
												 x, y, x + size, y + size );
	x = maxX - 5;//BORDER_CORNER_SIZE;
	y = originalY - 2;//BORDER_CORNER_SIZE;
	TheDisplay->drawImage(s_cornerUR,
												 x, y, x + size, y + size );
	x = originalX - 2;//BORDER_CORNER_SIZE;
	y = maxY - 5;//BORDER_CORNER_SIZE;
	TheDisplay->drawImage( s_cornerLL,
												 x, y, x + size, y + size );
	x = maxX - 5;//BORDER_CORNER_SIZE;
	y = maxY - 5;//BORDER_CORNER_SIZE;
	TheDisplay->drawImage(s_cornerLR,
												 x, y, x + size, y + size );
