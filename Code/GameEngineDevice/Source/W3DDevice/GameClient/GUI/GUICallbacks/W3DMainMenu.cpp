...
//	TheDisplay->setClipRegion(&clipRegion);
	...
	if(image)
	{
		...
		LinuxUIDrawImageRepeatedH(
			TheDisplay, image, originalX + 10, y, maxX - 10, y + size);
		LinuxUIDrawImageRepeatedH(
			TheDisplay, image, originalX + 10, y2, maxX - 10, y2 + size);
#else
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		LinuxUIDrawImageRepeatedH(
			TheDisplay, image, x, originalY + 10, x + size, maxY - 10);
		LinuxUIDrawImageRepeatedH(
			TheDisplay, image, x2, originalY + 10, x2 + size, maxY - 10);
#else
...
//		TheDisplay->setClipRegion(&clipRegion);
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		TheDisplay->drawImage(centerImage, leftEnd.x, start.y, rightStart.x, end.y);
#else
		for( i = 0; i < pieces; i++ )
		{
...
//		TheDisplay->setClipRegion(&clipRegion);
	...
	if( pData )
	{
		...
		if( pData->drawClock )
		{
			...
			/* Keep drawClock set: visual FPS > logic rate redraws without ControlBar update. */
		}
