...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	{
		Int fillEndX = origin.x + (Int)(s->numTicks * (s->position - s->minVal));
		if (fillEndX > origin.x + size.x) {
			fillEndX = origin.x + size.x;
		}
		if (s->position != s->minVal && fillEndX > start.x) {
			LinuxUIDrawImageRepeatedH(
				TheDisplay, fillSquare, start.x, start.y, fillEndX, end.y);
		}
		start.x = fillEndX + 2;
	}
#else
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	if (start.x < origin.x + size.x) {
		LinuxUIDrawImageRepeatedH(
			TheDisplay, blankSquare, start.x, start.y, origin.x + size.x, end.y);
	}
#else
	while(end.x < origin.x + size.x )
	{
...
//		smallCenterImageRight = smallCenterImageLeft	= GadgetSliderGetDisabledImageSmallCenter( window );
	...
	LinuxUIDrawImageRepeatedH(
		TheDisplay, centerImageLeft, leftEnd.x, start.y, rightStart.x, end.y);
	TheDisplay->setClipRegion(&clipRight);
	LinuxUIDrawImageRepeatedH(
		TheDisplay, centerImageRight, leftEnd.x, start.y, rightStart.x, end.y);
#else
	for( i = 0; i < pieces; i++ )
	{
