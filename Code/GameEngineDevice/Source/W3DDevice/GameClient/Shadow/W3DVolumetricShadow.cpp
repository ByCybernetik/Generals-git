...
DX8VertexBufferClass* shadowVertexBufferD3D=NULL;		///<dynamic shadow vertex buffer
DX8IndexBufferClass* shadowIndexBufferD3D=NULL;	///<dynamic shadow index buffer
...
static DX8VertexBufferClass* lastActiveVertexBuffer=NULL;
...
void W3DVolumetricShadow::RenderMeshVolume(Int meshIndex, Int lightIndex, const Matrix3D *meshXform)
{
	...
	Matrix4 worldLogical = mWorld.Transpose();
	...
	DX8Wrapper::Set_Transform(D3DTS_WORLD, worldLogical);

	if (vbSlot->m_VB->m_DX8VertexBuffer != lastActiveVertexBuffer)
	{	DX8Wrapper::Set_Vertex_Buffer(vbSlot->m_VB->m_DX8VertexBuffer);
		lastActiveVertexBuffer = vbSlot->m_VB->m_DX8VertexBuffer;
	}

	DX8Wrapper::Set_Index_Buffer(ibSlot->m_IB->m_DX8IndexBuffer, vbSlot->m_start);

	if (DX8Wrapper::_Is_Triangle_Draw_Enabled())
	{
		Debug_Statistics::Record_DX8_Polys_And_Vertices(numPolys,numVerts,ShaderClass::_PresetOpaqueShader);
		DX8Wrapper::Draw_Triangles(ibSlot->m_start, numPolys, 0, numVerts);
	}
...
void W3DVolumetricShadow::RenderDynamicMeshVolume(Int meshIndex, Int lightIndex, const Matrix3D *meshXform)
{
	...
	{
		DX8VertexBufferClass::AppendLockClass lockVtx(shadowVertexBufferD3D, nShadowVertsInBuf, numVerts);
		pvVertices = (SHADOW_DYNAMIC_VOLUME_VERTEX*)lockVtx.Get_Vertex_Array();
#ifdef SV_DEBUG
	srand(0x1345465);
#endif
	if(pvVertices)
	...
	{
		DX8IndexBufferClass::AppendLockClass lockIdx(shadowIndexBufferD3D, nShadowIndicesInBuf, numIndex);
		pvIndices = lockIdx.Get_Index_Array();
	...
	DX8Wrapper::Set_Index_Buffer(shadowIndexBufferD3D, nShadowStartBatchVertex);
	...
	{	DX8Wrapper::Set_Vertex_Buffer(shadowVertexBufferD3D);
	...
	{
		Debug_Statistics::Record_DX8_Polys_And_Vertices(numPolys,numVerts,ShaderClass::_PresetOpaqueShader);
		DX8Wrapper::Draw_Triangles(nShadowStartBatchIndex, numPolys, 0, numVerts);
	}
...
void W3DVolumetricShadow::RenderMeshVolumeBounds(Int meshIndex, Int lightIndex, const Matrix3D *meshXform)
{
	...
	DX8Wrapper::Set_Transform(D3DTS_WORLD, mWorld.Transpose());
...
void W3DVolumetricShadowManager::renderStencilShadows( void )
{
	...
	if (DX8Wrapper::Vulkan_Device_Active())
	{
		...
		ww3d_vulkan::Draw_Stencil_Darken_Quad(
...
//  DX8Wrapper::Set_DX8_Render_State( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );
	...
	if (DX8Wrapper::_Is_Triangle_Draw_Enabled() && !DX8Wrapper::Vulkan_Device_Active())
...
void W3DVolumetricShadowManager::renderShadows( Bool forceStencilFill )
{
	...
	if (m_shadowList && TheGlobalData->m_useShadowVolumes)
	{
		...
		/* All shadows forced off until Vulkan stencil volumes are fixed. */
		if (DX8Wrapper::Vulkan_Device_Active()) {
			static int once = 0;
			if (!once) {
				++once;
				fprintf(stderr, "shadow_diag: volumetric FORCED OFF\n");
				fflush(stderr);
			}
			return;
		}
	  ...
	  DX8Wrapper::Set_DX8_Render_State( D3DRS_ZENABLE,          TRUE );
...
#ifdef RENEGADE_VULKAN
		const Bool useNativeShadowPass = DX8Wrapper::Vulkan_Device_Active();
#else
		const Bool useNativeShadowPass = FALSE;
#endif

		if (useNativeShadowPass)
		{
#ifdef RENEGADE_VULKAN
			ww3d_vulkan::Shadow_Volume_Pass_Begin(TheW3DShadowManager->getStencilShadowMask());
#endif
		}
		else
		{
			...
			{	if (!DX8Wrapper::Vulkan_Device_Active())
					...
					oldColorWriteEnable = D3DCOLORWRITEENABLE_RED|D3DCOLORWRITEENABLE_GREEN|D3DCOLORWRITEENABLE_BLUE|D3DCOLORWRITEENABLE_ALPHA;
		...
		// change the stencil op to decrement / invert cull for back faces
		// Two-sided Vulkan path already applied INCR+DECRSAT in one draw — skip pass 2.
#ifdef RENEGADE_VULKAN
		const Bool skipDualPass = useNativeShadowPass && ww3d_vulkan::Shadow_Volume_Pass_Is_Two_Sided();
#else
		const Bool skipDualPass = FALSE;
#endif
		if (!skipDualPass)
		{
			if (useNativeShadowPass)
			{
#ifdef RENEGADE_VULKAN
				ww3d_vulkan::Shadow_Volume_Pass_Set_Mode(ww3d_vulkan::ShadowVolumeMode::Decrement);
#endif
			}
			else
			{
			...
			if (!useNativeShadowPass && shadowVertexBufferD3D)
			{	DX8Wrapper::Set_Vertex_Buffer(shadowVertexBufferD3D);
				lastActiveVertexBuffer = shadowVertexBufferD3D;
			}
		...
		{
#ifdef RENEGADE_VULKAN
			ww3d_vulkan::Shadow_Volume_Pass_End();
#endif
		}
		else
		{
...
{
	REF_PTR_RELEASE(shadowIndexBufferD3D);
	REF_PTR_RELEASE(shadowVertexBufferD3D);
...
Bool W3DVolumetricShadowManager::ReAcquireResources(void)
{
	...
	shadowVertexBufferD3D = NEW DX8VertexBufferClass(
	...
	shadowIndexBufferD3D = NEW DX8IndexBufferClass(
		SHADOW_INDEX_SIZE,
