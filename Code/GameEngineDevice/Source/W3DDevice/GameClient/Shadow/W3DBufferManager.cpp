...
void W3DBufferManager::freeAllSlots(void)
{
	...
	for (i=0; i<MAX_FVF; i++)
	{
		for (j=0; j<MAX_VB_SIZES; j++)
		{
			...
			while (vbSlot)
			{
				// Fully unlink from the owning vertex buffer's in-use list.
				if (vbSlot->m_prevSameVB)
					vbSlot->m_prevSameVB->m_nextSameVB=vbSlot->m_nextSameVB;
				else
					vbSlot->m_VB->m_usedSlots=vbSlot->m_nextSameVB;
	...
	for (j=0; j<MAX_IB_SIZES; j++)
	{
		...
		while (ibSlot)
		{
			// Fully unlink from the owning index buffer's in-use list.
			if (ibSlot->m_prevSameIB)
				ibSlot->m_prevSameIB->m_nextSameIB=ibSlot->m_nextSameIB;
			else
				ibSlot->m_IB->m_usedSlots=ibSlot->m_nextSameIB;
...
W3DBufferManager::W3DVertexBufferSlot *W3DBufferManager::getSlot(VBM_FVF_TYPES fvfType, Int size)
{
	...
	// A zero-size request would underflow the slot index array.
	if (size == 0)
		return NULL;
...
void W3DBufferManager::releaseSlot(W3DVertexBufferSlot *vbSlot)
{
	...
	// Unlink the slot from the vertex buffer's in-use list before returning it to the free pool.
	if (vbSlot->m_prevSameVB)
		vbSlot->m_prevSameVB->m_nextSameVB = vbSlot->m_nextSameVB;
	else
		vbSlot->m_VB->m_usedSlots = vbSlot->m_nextSameVB;
...
void W3DBufferManager::releaseSlot(W3DIndexBufferSlot *ibSlot)
{
	...
	// Unlink the slot from the index buffer's in-use list before returning it to the free pool.
	if (ibSlot->m_prevSameIB)
		ibSlot->m_prevSameIB->m_nextSameIB = ibSlot->m_nextSameIB;
	else
		ibSlot->m_IB->m_usedSlots = ibSlot->m_nextSameIB;
