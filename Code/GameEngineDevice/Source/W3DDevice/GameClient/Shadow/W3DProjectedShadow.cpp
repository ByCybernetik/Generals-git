...
extern DX8VertexBufferClass* shadowVertexBufferD3D;		///<DX8 vertex buffer
extern DX8IndexBufferClass*  shadowIndexBufferD3D;	///<DX8 index buffer
...
static Bool ShadowDecalRenderReady(void)
{
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active())
		return TRUE;
#endif
	return DX8Wrapper::_Get_D3D_Device8() != NULL;
}
...
DX8VertexBufferClass *shadowDecalVertexBufferD3D=NULL;		///<DX8 vertex buffer
DX8IndexBufferClass *shadowDecalIndexBufferD3D=NULL;	///<DX8 index buffer
...
{
	// Vulkan path: render target allocation uses the fork's helper, and the
	// shadow decal buffers are created through DX8IndexBufferClass/DX8VertexBufferClass
	// so they work when the D3D8 device is not available.
	ReleaseResources();
	...
	DEBUG_ASSERTCRASH(shadowDecalVertexBufferD3D == NULL, ("Acquire of existing shadow decal vertex buffer"));
	DEBUG_ASSERTCRASH(shadowDecalIndexBufferD3D == NULL, ("Acquire of existing shadow decal index buffer"));
	shadowDecalVertexBufferD3D = NEW DX8VertexBufferClass(SHADOW_DECAL_FVF, (unsigned short)SHADOW_DECAL_VERTEX_SIZE, DX8VertexBufferClass::USAGE_DYNAMIC);
	shadowDecalIndexBufferD3D = NEW DX8IndexBufferClass((unsigned short)SHADOW_DECAL_INDEX_SIZE, DX8IndexBufferClass::USAGE_DYNAMIC);
...
void W3DProjectedShadowManager::ReleaseResources(void)
{
	...
	REF_PTR_RELEASE(shadowDecalIndexBufferD3D);
	REF_PTR_RELEASE(shadowDecalVertexBufferD3D);
}
...
Int W3DProjectedShadowManager::renderProjectedTerrainShadow(W3DProjectedShadow *shadow, AABoxClass &box)
{
	...
	if (TheTerrainRenderObject)
	{
		...
		{
			DX8VertexBufferClass::WriteLockClass lock(shadowVertexBufferD3D);
			pvVertices = static_cast<SHADOW_VOLUME_VERTEX*>(lock.Get_Vertex_Array()) + nShadowVertsInBuf;
		...
		{
			DX8IndexBufferClass::WriteLockClass lock(shadowIndexBufferD3D);
			pvIndices = lock.Get_Index_Array() + nShadowIndicesInBuf;
		...
		DX8Wrapper::Set_Index_Buffer(shadowIndexBufferD3D,(unsigned short)nShadowStartBatchVertex);
...
//    DX8Wrapper::Set_DX8_Render_State( D3DRS_ALPHABLENDENABLE, FALSE );	//useful to see bounds
		...
		if (DX8Wrapper::_Is_Triangle_Draw_Enabled())
		{
			...
			DX8Wrapper::Draw_Triangles((unsigned short)nShadowStartBatchIndex,(unsigned short)numPolys,0,(unsigned short)numVerts);
		}
...
*/
	...
	DX8Wrapper::Set_Index_Buffer(shadowDecalIndexBufferD3D,(unsigned short)nShadowDecalStartBatchVertex);
	DX8Wrapper::Set_Transform(D3DTS_WORLD,mWorld);

	DX8Wrapper::Set_Vertex_Buffer(shadowDecalVertexBufferD3D);
...
//m_pDev->SetRenderState( D3DRS_ALPHABLENDENABLE, FALSE );	//useful to see bounds
	...
	if (DX8Wrapper::_Is_Triangle_Draw_Enabled())
	{
		...
		DX8Wrapper::Draw_Triangles((unsigned short)nShadowDecalStartBatchIndex,(unsigned short)nShadowDecalPolysInBatch,0,(unsigned short)nShadowDecalVertsInBatch);
	}
...
*/
		...
		DX8VertexBufferClass::WriteLockClass lock(shadowDecalVertexBufferD3D);
		pvVertices = static_cast<SHADOW_DECAL_VERTEX*>(lock.Get_Vertex_Array()) + nShadowDecalVertsInBuf;
...
*/
		...
		DX8IndexBufferClass::WriteLockClass lockib(shadowDecalIndexBufferD3D);
		pvIndices = lockib.Get_Index_Array() + nShadowDecalIndicesInBuf;
