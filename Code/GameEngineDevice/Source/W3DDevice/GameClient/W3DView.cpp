...
static void drawablePostDraw( Drawable *draw, void *userData )
{
	...
	ObjectShroudStatus ss =
		(!obj || !GlobalData_Shroud_Render_Enabled(TheGlobalData))
			? OBJECTSHROUD_CLEAR
...
Bool W3DView::updateCameraMovements()
{
	...
	/*
	 * Wall-clock step so scripted camera is smooth at any visual FPS (incl. map 20/25),
	 * while total duration stays the same as original 33ms/frame timing.
	 */
	Real frameDelta = 1.0f;
	Int pathDtMs = TheW3DFrameLengthInMsec;
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	{
		static DWORD s_lastCamWallMs = 0;
		const DWORD wallNow = timeGetTime();
		Real dtMs = (s_lastCamWallMs != 0) ? (Real)(wallNow - s_lastCamWallMs) : (Real)TheW3DFrameLengthInMsec;
		s_lastCamWallMs = wallNow;
		if (dtMs < 1.0f)
			dtMs = 1.0f;
		else if (dtMs > 100.0f)
			dtMs = 100.0f;
		frameDelta = dtMs / (Real)TheW3DFrameLengthInMsec;
		pathDtMs = REAL_TO_INT(dtMs + 0.5f);
		if (pathDtMs < 1)
			pathDtMs = 1;
	}
#endif
	...
	{
		zoomCameraOneFrame(frameDelta);
		didUpdate = true;
	}
	if (m_doingPitchCamera)
	{
		pitchCameraOneFrame(frameDelta);
		didUpdate = true;
	}
	if (m_doingRotateCamera) {	
		m_previousLookAtPosition = *getPosition();
		rotateCameraOneFrame(frameDelta);
		didUpdate = true;
	} else if (m_doingMoveCameraOnWaypointPath) {
		m_previousLookAtPosition = *getPosition();
		moveAlongWaypointPath(pathDtMs);
		didUpdate = true;
	}
...
//	Int elapsedTimeMs = TheW3DFrameLengthInMsec; // Assume a constant time flow.  It just works out better.  jba.
	...
	if (cameraLock != INVALID_ID)
	{
		...
		else
		{
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
			/*
			 * Tether/follow catch-up must run at logic rate when inside play radius.
			 * Far approach + script snapToCameraLock used to teleport (~365 units in one
			 * visual frame) or half-step every visual frame — both read as start jerk.
			 */
			static UnsignedInt s_lockLogicFrame = ~0;
			static Real s_lockLogicLookAtX = 0.0f;
			static Real s_lockLogicLookAtY = 0.0f;
			static Bool s_haveLockLogicLookAt = false;
			static Bool s_lockFarEase = false;
			static DWORD s_lockLastWallMs = 0;
			const UnsignedInt lockFrame = TheGameClient ? TheGameClient->getFrame() : 0;
			const Bool newLockLogicFrame = (lockFrame != s_lockLogicFrame);
			if (newLockLogicFrame)
				s_lockLogicFrame = lockFrame;
			const DWORD lockWallNow = timeGetTime();
			Real lockDtMs = (s_lockLastWallMs != 0) ? (Real)(lockWallNow - s_lockLastWallMs) : (Real)TheW3DFrameLengthInMsec;
			s_lockLastWallMs = lockWallNow;
			if (lockDtMs < 1.0f)
				lockDtMs = 1.0f;
			else if (lockDtMs > 100.0f)
				lockDtMs = 100.0f;
			if (followFactor < 0)
			{
				s_haveLockLogicLookAt = false;
				s_lockFarEase = false;
				followFactor = 0.05f;
			}
			else if (newLockLogicFrame)
			{
				followFactor += 0.05f;
			...
			{	Coord3D logicPos = *cameraLockObj->getPosition();
				Coord3D visualPos = logicPos;
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
				/*
				 * L: Training uses LOCK_TETHER. Feeding visual pos into tether math made the
				 * play-radius deadzone fight translation lerp (heli slides, cam lags). Keep
				 * tether/follow vs logic, then add the same visual-logic offset the drawable uses.
				 */
				if (Drawable *lockDraw = cameraLockObj->getDrawable())
					lockDraw->getVisualDrawPosition(visualPos);
#endif
				Coord3D objpos = logicPos;
				Coord3D curpos = *getPosition();
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
				/* setPosition stores lookat+visual offset; catch-up must start from logic lookat. */
				if (s_haveLockLogicLookAt)
				{
					curpos.x = s_lockLogicLookAtX;
					curpos.y = s_lockLogicLookAtY;
				}
#endif
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
				{
					...
					const Bool outsidePlay = (curDistSqr >= snapThreshSqr);
					/*
					 * N: Script snapToCameraLock teleported ~365 units in one frame (heli-test3).
					 * Convert snap into far ease-in instead of teleport.
					 */
					if (m_snapImmediate)
					{
					...
					if (s_lockFarEase || outsidePlay)
					{
						/* Wall-clock exponential chase — smooth at 60 FPS from far lock-on. */
						const Real k = 6.0f;
						Real a = 1.0f - expf(-k * (lockDtMs * 0.001f));
						if (a < 0.0f) a = 0.0f;
						if (a > 1.0f) a = 1.0f;
						curpos.x += (objpos.x - curpos.x) * a;
						curpos.y += (objpos.y - curpos.y) * a;
						curDistSqr = sqr(curpos.x - objpos.x) + sqr(curpos.y - objpos.y);
						if (curDistSqr < snapThreshSqr)
							s_lockFarEase = false;
					}
					else if (newLockLogicFrame || !s_haveLockLogicLookAt)
					{
					...
					/* else: hold stored logic lookat between logic ticks */

					s_lockLogicLookAtX = curpos.x;
					s_lockLogicLookAtY = curpos.y;
					s_haveLockLogicLookAt = true;
				}
#else
				{
				Real curDistSqr = sqr(curpos.x - objpos.x) + sqr(curpos.y - objpos.y);
...
//					Real ratio = 1.0f - snapThreshSqr/curDistSqr;
...
#endif
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
				/* Match drawable translation lerp so tether slack is vs logic, not vs lerped mesh. */
				curpos.x += (visualPos.x - logicPos.x);
				curpos.y += (visualPos.y - logicPos.y);
#endif
				...
				if (m_lockType == LOCK_FOLLOW)
				{
					...
					{
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
						if (newLockLogicFrame || m_snapImmediate)
#endif
						{
						Matrix3D camXForm;
	...
	if (!(TheScriptEngine->isTimeFrozenDebug()/* || TheScriptEngine->isTimeFrozenScript()*/) && !TheGameLogic->isGamePaused()) {
		/*
		 * Scripted camera advances on wall-clock (see updateCameraMovements) so motion
		 * stays smooth when visual FPS is 20/25 (map) or 60+. Duration matches original.
		 */
		{
	...
	//
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	static UnsignedInt s_lastShakeFrame = ~0;
	const UnsignedInt shakeFrame = TheGameClient ? TheGameClient->getFrame() : 0;
	const Bool doShakeStep = (shakeFrame != s_lastShakeFrame);
	if (doShakeStep)
		s_lastShakeFrame = shakeFrame;
	if (doShakeStep && m_shakeIntensity > 0.01f)
#else
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void W3DView::rotateCameraOneFrame(Real frameDelta)
{
	m_rcInfo.curFrame += frameDelta;
	...
	if (m_rcInfo.curFrame <= m_rcInfo.numFrames)
	{
		...
		Real factor = m_rcInfo.curFrame / (Real)m_rcInfo.numFrames;
		if (m_rcInfo.trackObject)
		{
			...
			if (dir.Length()>=0.1)
			{
				...
				Real denom = (Real)m_rcInfo.numFrames - m_rcInfo.curFrame + frameDelta;
				if (denom < 0.001f)
					denom = 0.001f;
				factor = frameDelta / denom;
		...
		{
			m_angle += m_rcInfo.angle * frameDelta;
		}
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void W3DView::zoomCameraOneFrame(Real frameDelta)
{
	m_zcInfo.curFrame += frameDelta;
	if (TheGlobalData->m_disableCameraMovement) {
	...
	{
		// not just holding; do the camera adjustment
		Real factor = m_zcInfo.curFrame / (Real)m_zcInfo.numFrames;
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void W3DView::pitchCameraOneFrame(Real frameDelta)
{
	m_pcInfo.curFrame += frameDelta;
	if (TheGlobalData->m_disableCameraMovement) {
	...
	{
		// not just holding; do the camera adjustment
		Real factor = m_pcInfo.curFrame / (Real)m_pcInfo.numFrames;
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/* Shutter skipped position updates while time advanced — looks like stepped cuts at 60 FPS. */
#else
