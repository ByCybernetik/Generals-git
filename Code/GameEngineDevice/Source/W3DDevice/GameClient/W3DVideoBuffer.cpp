...
void		W3DVideoBuffer::unlock( void )
{
	...
	{
#if defined(RENEGADE_VULKAN)
		if (DX8Wrapper::Vulkan_Device_Active() && m_texture != NULL) {
			const ww3d_vulkan::VkCpuSurface *cpu =
				static_cast<const ww3d_vulkan::VkCpuSurface *>(m_surface->Peek_Vulkan_Cpu_Surface());
			if (cpu != NULL && cpu->Pixels() != NULL && m_width > 0 && m_height > 0) {
				const bool uploaded = ww3d_vulkan::Upload_Procedural_Texture_Rgb565(
					m_texture,
					TypeToW3DFormat(m_format),
					cpu->Pixels(),
					(int)cpu->Pitch(),
					m_width,
					m_height);
