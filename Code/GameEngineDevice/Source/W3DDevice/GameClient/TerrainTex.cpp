...
static Bool Use_Single_Pass_Terrain_Hack(void)
{
	if (TheGlobalData == NULL || TheGlobalData->m_multiPassTerrain) {
		return FALSE;
	}
#if defined(RENEGADE_VULKAN)
...
struct TerrainSurfaceLock {
	...
	bool Is_Valid(void) const { return surface != NULL && bits != NULL; }
};

static TerrainSurfaceLock Lock_Terrain_Surface(TextureClass *texture)
{
	TerrainSurfaceLock lock;
	if (texture == NULL) {
		return lock;
	}

	lock.surface = texture->Get_Surface_Level(0);
	if (lock.surface == NULL) {
		return lock;
	}

	SurfaceClass::SurfaceDescription desc;
	lock.surface->Get_Description(desc);
	lock.width = desc.Width;
	lock.height = desc.Height;
	lock.format = desc.Format;
	lock.bits = lock.surface->Lock(&lock.pitch);
	return lock;
}

static void Unlock_Terrain_Surface(TextureClass *texture, TerrainSurfaceLock &lock)
{
	if (lock.surface == NULL) {
		return;
	}
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		const ww3d_vulkan::VkCpuSurface *cpu =
			static_cast<const ww3d_vulkan::VkCpuSurface *>(lock.surface->Peek_Vulkan_Cpu_Surface());
		if (cpu != NULL) {
			ww3d_vulkan::Upload_Procedural_Texture_From_Cpu_Surface(texture, cpu);
		}
	}
#endif

	REF_PTR_RELEASE(lock.surface);
	lock.surface = NULL;
	lock.bits = NULL;
}

static void Filter_Terrain_Texture_Mips(TextureClass *texture)
{
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		return;
	...
	if (texture != NULL && texture->Peek_DX8_Texture() != NULL) {
		DX8_ErrorCode(D3DXFilterTexture(texture->Peek_DX8_Texture(), NULL, 0, D3DX_FILTER_BOX));
	}
}
...
{
	TerrainSurfaceLock lock = Lock_Terrain_Surface(this);
	if (!lock.Is_Valid()) {
		return false;
	}

	if (lock.width < TEXTURE_WIDTH) {
		Unlock_Terrain_Surface(this, lock);
		if (lock.width == 256) {
	...
	Int tilesPerRow = lock.width/(2*TILE_PIXEL_EXTENT+TILE_OFFSET);
	tilesPerRow *= 2;
#ifdef _DEBUG
	DEBUG_ASSERTCRASH((Int)lock.width >= tilePixelExtent*tilesPerRow, ("Bitmap too small."));
#endif
	if (lock.format == WW3D_FORMAT_A1R5G5B5) {
		Int tileNdx;
		Int pixelBytes = 2;
		for (tileNdx=0; tileNdx < htMap->m_numBitmapTiles; tileNdx++) {
			...
			for (j=0; j<tilePixelExtent; j++) {
				...
				UnsignedByte *pBGRX = ((UnsignedByte*)lock.bits) + row*lock.pitch;
		...
		for (texClass=0; texClass<htMap->m_numTextureClasses; texClass++) {
			...
			for (j=0; j<4; j++) {
				...
				memcpy(target, target+width*lock.pitch, (width+8)*pixelBytes);
				// copy after.
				row = origin.y+j;
				pBGRX = ((UnsignedByte*)lock.bits) + row*lock.pitch;
				target = pBGRX+(origin.x-4)*pixelBytes; 
				memcpy(target+width*lock.pitch, target, (width+8)*pixelBytes);
			}
	...
	Unlock_Terrain_Surface(this, lock);
	Filter_Terrain_Texture_Mips(this);
#if !defined(RENEGADE_VULKAN)
	if (Peek_DX8_Texture() && TheWritableGlobalData->m_textureReductionFactor) {
		Peek_DX8_Texture()->SetLOD(TheWritableGlobalData->m_textureReductionFactor);
	}
#else
	if (!DX8Wrapper::Vulkan_Device_Active() && Peek_DX8_Texture() && TheWritableGlobalData->m_textureReductionFactor) {
		Peek_DX8_Texture()->SetLOD(TheWritableGlobalData->m_textureReductionFactor);
	}
#endif
	return(lock.height);
...
int TerrainTextureClass::update256(WorldHeightMap *htMap)
{
	...
	Int tilesPerRow = lock.width/(2*tilePixelExtent+tileOffset);
	tilesPerRow *= 2;
	Int numRows = lock.height/(tilePixelExtent+tileOffset);

#ifdef _DEBUG
	assert(tilesPerRow*numRows >= htMap->m_numBitmapTiles);
	assert((Int)lock.width >= tilePixelExtent*tilesPerRow);
#endif
	if (lock.format == WW3D_FORMAT_A1R5G5B5) {
		Int cellX, cellY;
#if 0
		...
		for (cellY = 0; cellY < numRows; cellY++) {
				...
				for (j=0; j<tilePixelExtent; j++) {
					...
					if (cellX&1) {
						...
						if (cellX == tilesPerRow-1) {
							...
							usedWidth *= pixelBytes;
							bytesToCopy += lock.width*pixelBytes - usedWidth;
						}
						memcpy(pBGRX+(2*tilePixelExtent*pixelBytes), pBGRX, bytesToCopy);
						// Copy after. 
						pBGRX -= tilePixelExtent*pixelBytes;
 		...
 		for (cellY = 0; cellY < numRows; cellY++) {
			Int rowBytes = lock.pitch; 
			Int row = cellY*tilePixelExtent;
			row += tileOffset/2; 
			row += tileOffset*(cellY/2);
			if ( (cellY&1) == 0) {
				UnsignedByte *pBGRX = ((UnsignedByte*)lock.bits) + row*rowBytes;
				row += 2*tilePixelExtent;
				UnsignedByte *pBase = ((UnsignedByte*)lock.bits) + row*rowBytes;
				memcpy(pBGRX-rowBytes, pBase-rowBytes, rowBytes);
			} else {
				UnsignedByte *pBase = ((UnsignedByte*)lock.bits) + (row-tilePixelExtent)*rowBytes;
				row += tilePixelExtent;
				UnsignedByte *pBGRX = ((UnsignedByte*)lock.bits) + row*rowBytes;
...
{ 
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		...
		/* Share base GPU texture like D3D AddRef path — do not own/destroy it. */
		Set_Vulkan_Texture(pBaseTex->Peek_Vulkan_Texture(), false);
		Set_Dimensions(pBaseTex->Get_Width(), pBaseTex->Get_Height());
		TextureFormat = pBaseTex->Get_Texture_Format();
...
void AlphaTerrainTextureClass::Apply(unsigned int stage)
{
	...
	if (stage==0) {
		...
		if (Use_Single_Pass_Terrain_Hack())
		{
...
#if defined(RENEGADE_VULKAN)
	/*
	 * Do not set ViewInv*Scale via D3DTS_TEXTUREn — Extract_Tex_Mat breaks when
	 * the camera rotates. Mesh cloud/noise uses world.xy * stretch like terrain.
	 */
	if (DX8Wrapper::Vulkan_Device_Active()) {
		DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_ADDRESSU, D3DTADDRESS_WRAP);
		DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_ADDRESSV, D3DTADDRESS_WRAP);
		if (stage < 2) {
			ww3d_vulkan::Set_World_XY_Tex_Projection(stage, STRETCH_FACTOR, 0.0f, 0.0f);
		}
...
int AlphaEdgeTextureClass::update(WorldHeightMap *htMap)
{
	...
	if (lock.format == WW3D_FORMAT_A8R8G8B8) {
#if 1
#if 1
		Int cellX, cellY;
		for (cellX = 0; (UnsignedInt)cellX < lock.width; cellX++) {
			for (cellY = 0; cellY < (Int)lock.height; cellY++) {
				UnsignedByte *pBGR = ((UnsignedByte *)lock.bits)+(cellY*lock.pitch+cellX*4);
				pBGR[2] = 255-cellY/2;
				pBGR[0] = cellX/2;
...
#if defined(RENEGADE_VULKAN)
	/*
	 * Same as LightMap: world-XY projection for mesh SPIR-V. Skip ViewInv*Scale
	 * which Vulkan_Apply_Per_Draw_State would otherwise re-apply every draw and
	 * undo RoadShader / CloudTextureShader UV setup.
	 */
	if (DX8Wrapper::Vulkan_Device_Active()) {
		DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_ADDRESSU, D3DTADDRESS_WRAP);
		DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_ADDRESSV, D3DTADDRESS_WRAP);
		if (stage < 2) {
			ww3d_vulkan::Set_World_XY_Tex_Projection(
				stage, STRETCH_FACTOR, m_xOffset, m_yOffset);
		}
		if (stage == 0) {
			DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
			DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_COLORARG2, D3DTA_DIFFUSE);
...
void CloudMapTerrainTextureClass::restore(void)
{
	...
	DX8Wrapper::Set_DX8_Texture_Stage_State( 1, D3DTSS_COLORARG2, D3DTA_DIFFUSE );
	/*
	 * On Vulkan, MODULATE here sticks in g_tex_stage1 via On_Tss and can
	 * multiply mesh draws by a leftover stage1 texel (sparkle on bunkers).
	 * D3D restored MODULATE as a harmless default; SPIR-V must DISABLE.
	 */
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		DX8Wrapper::Set_DX8_Texture_Stage_State( 1, D3DTSS_COLOROP,   D3DTOP_DISABLE );
