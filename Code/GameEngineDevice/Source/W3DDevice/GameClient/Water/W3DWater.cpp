...
static void Apply_Vulkan_Water_Cull_Disable(void)
{
	if (!DX8Wrapper::Vulkan_Device_Active()) {
		return;
	}
	ShaderClass water_shader;
	DX8Wrapper::Get_Shader(water_shader);
	water_shader.Set_Cull_Mode(ShaderClass::CULL_MODE_DISABLE);
	DX8Wrapper::Set_Shader(water_shader);
}
#endif
...
void WaterRenderObjClass::setupJbaWaterShader(void) 
{
	...
	TextureClass *alphaEdge = m_riverAlphaEdge ? m_riverAlphaEdge : m_whiteTexture;
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		/*
		 * D3D binds alpha-edge on stage 3, but DX8Wrapper/Vulkan only expose
		 * stages 0-1 (MAX_TEXTURE_STAGES). Remap stage 3 -> 1 and enable
		 * detail-alpha so mesh_textured.frag samples v_uv2.
		 */
		const unsigned alphaStage = 1;
		ShaderClass waterShader = ShaderClass::_PresetAlphaShader;
		waterShader.Set_Post_Detail_Alpha_Func(ShaderClass::DETAILALPHA_DETAIL);
		DX8Wrapper::Set_Shader(waterShader);
		if (alphaEdge != nullptr) {
			DX8Wrapper::Set_Texture(alphaStage, alphaEdge);
		}
		DX8Wrapper::Set_DX8_Texture_Stage_State(alphaStage, D3DTSS_ADDRESSU, D3DTADDRESS_WRAP);
		DX8Wrapper::Set_DX8_Texture_Stage_State(alphaStage, D3DTSS_ADDRESSV, D3DTADDRESS_WRAP);
		DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_TEXCOORDINDEX, 0);
		DX8Wrapper::Set_DX8_Texture_Stage_State(alphaStage, D3DTSS_TEXCOORDINDEX, 1);
		DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_MINFILTER, D3DTEXF_LINEAR);
		DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_MAGFILTER, D3DTEXF_LINEAR);
		DX8Wrapper::Set_DX8_Texture_Stage_State(alphaStage, D3DTSS_MINFILTER, D3DTEXF_LINEAR);
		DX8Wrapper::Set_DX8_Texture_Stage_State(alphaStage, D3DTSS_MAGFILTER, D3DTEXF_LINEAR);
		return;
	}
#endif

	if (alphaEdge)
		DX8Wrapper::_Get_D3D_Device8()->SetTexture(3, alphaEdge->Peek_DX8_Texture());
...
WaterRenderObjClass::WaterRenderObjClass(void)
{
	...
	m_dy=0;
	m_indexBuffer=NULL;
	m_waterGridIndexBuffer=NULL;
	m_waterGridVertexBuffer=NULL;
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active() && !doStatic) {
		m_numVertices = sizeX * sizeY;
		REF_PTR_RELEASE(m_waterGridVertexBuffer);
		if (m_numVertices > 0 && m_numVertices <= 65535) {
			m_waterGridVertexBuffer = NEW_REF(
				DX8VertexBufferClass,
				(WATER_MESH_FVF,
				 (unsigned short)m_numVertices,
				 DX8VertexBufferClass::USAGE_DYNAMIC));
		}
		return m_waterGridVertexBuffer ? S_OK : E_FAIL;
	}
#endif
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		REF_PTR_RELEASE(m_waterGridIndexBuffer);
		const unsigned short index_count = (unsigned short)(m_numIndices + 2);
		m_waterGridIndexBuffer = NEW_REF(DX8IndexBufferClass, (index_count));
		if (m_waterGridIndexBuffer == nullptr) {
			return E_FAIL;
		}
		{
			DX8IndexBufferClass::WriteLockClass lock(m_waterGridIndexBuffer);
			pIndices = (WORD *)lock.Get_Index_Array();
			Int i, j, k;
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active() &&
			m_waterGridVertexBuffer != nullptr &&
			m_waterGridIndexBuffer != nullptr) {
		{
			DX8VertexBufferClass::WriteLockClass vb_lock(m_waterGridVertexBuffer);
			MaterMeshVertexFormat *vb =
				(MaterMeshVertexFormat *)vb_lock.Get_Vertex_Array();
			for (j = 0, pData = m_meshData + mx + 2 + 1; j < my; j++, pData += 2) {
				Real y = (float)j * cellSizeY;
				Real v1Offset = m_riverVOrigin + (float)j * vScale +
		...
		DX8Wrapper::Set_Index_Buffer(m_waterGridIndexBuffer, 0);
		DX8Wrapper::Set_Vertex_Buffer(m_waterGridVertexBuffer);
		DX8Wrapper::Apply_Render_State_Changes();
		DX8Wrapper::Draw_Strip(
			0,
			(unsigned short)(m_numIndices - 2),
			0,
		...
		m_shaderClass.Set_Cull_Mode(oldCullMode);
...
#define HEIGHT_TO_USE (0.5f)
	...
	setupJbaWaterShader();
#if defined(RENEGADE_VULKAN)
	if (m_riverWaterPixelShader && !DX8Wrapper::Vulkan_Device_Active())
#else
	if (m_riverWaterPixelShader)
#endif
		DX8Wrapper::_Get_D3D_Device8()->SetPixelShader(m_riverWaterPixelShader);
...
#if defined(RENEGADE_VULKAN)
	Apply_Vulkan_Water_Cull_Disable();
#endif
	...
	if (TheTerrainRenderObject->getShroud())
	{
		...
		if (DX8Wrapper::Vulkan_Device_Active()) {
			/*
			 * ST_SHROUD_TEXTURE uses camera-space UV on the mesh path — same
			 * broken shroud redraw as terrain; second pass paints water black.
			 */
		} else
#endif
		{
...
void WaterRenderObjClass::setupFlatWaterShader(void) 
{
	...
	if (m_trapezoidWaterPixelShader)
	{
		...
		if (TheTerrainRenderObject->getShroud())
		{
			...
			DX8Wrapper::Set_DX8_Texture_Stage_State( 3, D3DTSS_MINFILTER, D3DTEXF_LINEAR );
			DX8Wrapper::Set_DX8_Texture_Stage_State( 3, D3DTSS_MAGFILTER, D3DTEXF_LINEAR );
			DX8Wrapper::Set_DX8_Texture_Stage_State( 3, D3DTSS_ADDRESSU, D3DTADDRESS_CLAMP);
			DX8Wrapper::Set_DX8_Texture_Stage_State( 3, D3DTSS_ADDRESSV, D3DTADDRESS_CLAMP);
			//Shroud shader uses z-compare of EQUAL which wouldn't work on water because it doesn't
			//write to the zbuffer.  Change to LESSEQUAL.
			DX8Wrapper::Set_DX8_Render_State(D3DRS_ZFUNC, D3DCMP_LESSEQUAL);
		}
...
#if defined(RENEGADE_VULKAN)
			if (DX8Wrapper::Vulkan_Device_Active()) {
				/* ST_SHROUD_TEXTURE mesh pass — see drawRiverWater comment. */
			} else
#endif
			{
			W3DShaderManager::setTexture(0,TheTerrainRenderObject->getShroud()->getShroudTexture());
