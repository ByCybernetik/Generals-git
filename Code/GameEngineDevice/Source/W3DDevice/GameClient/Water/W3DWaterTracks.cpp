...
#include "ww3d2_vulkan/vk_dx8_bridge.h"
#endif
...
Int WaterTracksObj::render(DX8VertexBufferClass	*vertexBuffer, Int batchStart)
{
	...
	const Int vertexSize = vertexBuffer->FVF_Info().Get_FVF_Size();
	Int lockStart = batchStart;
	...
	{
		vb = reinterpret_cast<VertexFormatXYZDUV1 *>(ww3d_vulkan::VB_Lock(
			vertexBuffer, lockStart * vertexSize, m_x * m_y * vertexSize));
		if (!vb)
			return batchStart;
	}
	else
#endif
	{
		IDirect3DVertexBuffer8 *d3d = vertexBuffer->Get_DX8_Vertex_Buffer();
		if (!d3d)
			return batchStart;
		DWORD lockFlags = lockStart == 0 ? D3DLOCK_DISCARD : D3DLOCK_NOOVERWRITE;
		if(d3d->Lock(
			lockStart * vertexSize, m_x * m_y * vertexSize,
			(unsigned char**)&vb, lockFlags) != D3D_OK)
			return batchStart;
	}
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active())
		ww3d_vulkan::VB_Unlock(vertexBuffer);
	else
#endif
				...
				if (addPointReset)
				{
					...
					else
					{
						...
						{//	track->init(1.5f*MAP_XY_FACTOR,Vector2(terrainPointStart.x,terrainPointStart.y),Vector2(terrainPointEnd.x,terrainPointEnd.y),"wave256.tga");
							...
							Vector2 startVec(midPoint.X, midPoint.Y);
							Vector2 endVec(dirMidPoint.X, dirMidPoint.Y);
							track->init(waveTypeInfo[currentWaveType].m_finalHeight,waveTypeInfo[currentWaveType].m_finalWidth,startVec,endVec,waveTypeInfo[currentWaveType].m_textureName,0);
							...
							if (waveTypeInfo[currentWaveType].m_secondWaveTimeOffset)
							{
								...
								if (track2)
								{
									track2->init(waveTypeInfo[currentWaveType].m_finalHeight,waveTypeInfo[currentWaveType].m_finalWidth,startVec,endVec,waveTypeInfo[currentWaveType].m_textureName,waveTypeInfo[currentWaveType].m_secondWaveTimeOffset);
								}
							}
