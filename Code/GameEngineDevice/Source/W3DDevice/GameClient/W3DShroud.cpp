...
static WW3DFormat Shroud_Src_Format(void)
{
...
/** Pack FOW A4R4G4B4 pixel. Default white ShroudColor → black RGB so alpha blend darkens. */
static UnsignedShort Pack_Fow_Shroud_Pixel(W3DShroudLevel level)
{
	Int redVal = (Int)(TheGlobalData->m_shroudColor.red * 255.0f + 0.5f);
	Int greenVal = (Int)(TheGlobalData->m_shroudColor.green * 255.0f + 0.5f);
	Int blueVal = (Int)(TheGlobalData->m_shroudColor.blue * 255.0f + 0.5f);
	if (redVal >= 250 && greenVal >= 250 && blueVal >= 250) {
		redVal = greenVal = blueVal = 0;
	}
	Int alphaVal = 255 - (Int)level;
	return (UnsignedShort)(((blueVal >> 4) & 0xf) |
		(((greenVal >> 4) & 0xf) << 4) |
		(((redVal >> 4) & 0xf) << 8) |
		(((alphaVal >> 4) & 0xf) << 12));
}

static void Copy_Shroud_Rect_To_Texture(
	SurfaceClass *src,
	TextureClass *dst_texture,
	SurfaceClass *dst_surface,
	WW3DFormat src_format,
	const RECT &src_rect,
	const POINT &dst_point)
{
	const int copy_w = src_rect.right - src_rect.left;
	const int copy_h = src_rect.bottom - src_rect.top;
	if (copy_w <= 0 || copy_h <= 0 || src == nullptr) {
		return;
	}

#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active() && dst_texture != nullptr) {
		const ww3d_vulkan::VkCpuSurface *cpu =
			static_cast<const ww3d_vulkan::VkCpuSurface *>(src->Peek_Vulkan_Cpu_Surface());
		if (cpu != nullptr && cpu->Pixels() != nullptr && cpu->Width() > 0) {
			const unsigned bpp = cpu->Pitch() / cpu->Width();
			const unsigned char *src_ptr =
				cpu->Pixels() + (size_t)src_rect.top * (size_t)cpu->Pitch() + (size_t)src_rect.left * bpp;
			ww3d_vulkan::Upload_Procedural_Texture_Rgb565_Region(
				dst_texture,
				src_format,
				src_ptr,
				(int)cpu->Pitch(),
				(unsigned)copy_w,
				(unsigned)copy_h,
				(unsigned)dst_point.x,
				(unsigned)dst_point.y);
		}
		return;
	}
#endif
	...
	if (dst_surface != nullptr) {
		DX8Wrapper::_Copy_DX8_Rects(
			src->Peek_D3D_Surface(),
			&src_rect,
			1,
			dst_surface->Peek_D3D_Surface(),
			&dst_point);
	}
}
...
{
	DEBUG_ASSERTCRASH( m_pSrcSurface == NULL, ("ReAcquire of existing shroud textures"));
  ...
  //memory texture to a known value because you can't lock it - only copy into it.
	...
	const WW3DFormat src_format = Shroud_Src_Format();
	m_pSrcSurface = NEW_REF(SurfaceClass, (srcWidth, srcHeight, src_format));

	DEBUG_ASSERTCRASH( m_pSrcSurface != NULL, ("Failed to Allocate Shroud Src Surface"));

	int src_pitch = 0;
	m_srcTextureData = m_pSrcSurface->Lock(&src_pitch);
	m_pSrcSurface->Unlock();
	m_srcTexturePitch = (UnsignedInt)src_pitch;
...
{
	DEBUG_ASSERTCRASH( m_pSrcSurface != NULL, ("Reading empty shroud"));
...
{
	DEBUG_ASSERTCRASH( m_pSrcSurface != NULL, ("Writing empty shroud.  Usually means that map failed to load."));
	...
	if (x < m_numCellsX && y < m_numCellsY)
	{
		...
		if (GlobalData_Fog_Of_War_Render_Enabled(TheGlobalData))
		{
			*(UnsignedShort *)((Byte *)m_srcTextureData + x*2 + y*m_srcTexturePitch) =
				Pack_Fow_Shroud_Pixel(level);
		}
		else
		{
#ifdef DO_FOG_INTERPOLATION
...
void W3DShroud::fillShroudData(W3DShroudLevel level)
{
	...
	if (GlobalData_Fog_Of_War_Render_Enabled(TheGlobalData))
	{
		pixel = Pack_Fow_Shroud_Pixel(level);
	}
	else
	{
		UnsignedInt bluepixel = (UnsignedInt)((Real)level*((Real)(TheGlobalData->m_shroudColor.getAsInt()&0xff)/255.0f));
...
void W3DShroud::render(CameraClass *cam)
{
	...
	const Bool fogOfWarOn = GlobalData_Fog_Of_War_Render_Enabled(TheGlobalData);
	if (fogOfWarOn != m_drawFogOfWar)
	{	
		...
		ThePartitionManager->refreshShroudForLocalPlayer();
		m_drawFogOfWar=fogOfWarOn;
		m_clearDstTexture=TRUE;
	}

	DEBUG_ASSERTCRASH( m_pSrcSurface != NULL, ("Updating unallocated shroud texture"));
