...
static Bool W3D_Bilinear_Or_Trilinear_Terrain(void)
{
	return TheGlobalData != nullptr &&
		(TheGlobalData->m_bilinearTerrainTex || TheGlobalData->m_trilinearTerrainTex);
}

static Bool W3D_Trilinear_Terrain(void)
{
	return TheGlobalData != nullptr && TheGlobalData->m_trilinearTerrainTex;
}

static void W3D_Bind_Shader_Stage_Texture(unsigned stage, TextureClass *texture)
{
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		/* Keep render_state.Textures in sync — Draw_Primitive::Prepare_Draw
		 * re-binds rs.Textures and would overwrite custom shader binds. */
		DX8Wrapper::Set_Texture(stage, texture);
		if (texture != nullptr) {
			ww3d_vulkan::Texture_Stage_Bind(texture, stage);
		} else {
	...
	if (device != nullptr) {
		device->SetTexture(
			stage,
			texture != nullptr ? texture->Peek_DX8_Texture() : nullptr);
	}
}

static void W3D_Bind_DX8_Texture(unsigned stage, IDirect3DBaseTexture8 *texture)
{
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		/* Keep the DX8 texture slot in sync so the Vulkan backend can resolve
		 * the VkTexture when this render-target texture is bound. */
...
public:
	...
	virtual void reset(void) {
		...
		W3D_Bind_Shader_Stage_Texture(0, NULL);
		W3D_Bind_Shader_Stage_Texture(1, NULL);};
...
Bool ScreenBWFilter::postRender(enum FilterModes mode, Coord2D &scrollDelta,Bool &doExtraRender)
{
	...
	W3D_Bind_DX8_Texture(0, tex);	//previously rendered frame inside this texture
...
{
	W3D_Bind_DX8_Texture(0, NULL);	//previously rendered frame inside this texture
...
Bool ScreenCrossFadeFilter::postRender(enum FilterModes mode, Coord2D &scrollDelta,Bool &doExtraRender)
{
	...
	{	W3D_Bind_Shader_Stage_Texture(1, m_fadePatternTexture);
...
Int ShroudTextureShader::set(Int stage)
{
	...
	if (stage == 0)
	{
	if (GlobalData_Fog_Of_War_Render_Enabled(TheGlobalData))
...
#if defined(RENEGADE_VULKAN)
	/*
	 * Roads/bridges often draw with ZWRITE off; EQUAL discards the whole
	 * shroud pass on Vulkan. LEQUAL still masks to the surface depth.
	 */
	if (DX8Wrapper::Vulkan_Device_Active()) {
	...
	{	///@todo: All this code really only need to be done once per camera/view.  Find a way to optimize it out.
		...
		float scaleX = 1.0f/(width*shroud->getTextureWidth());
		float scaleY = 1.0f/(height*shroud->getTextureHeight());

#if defined(RENEGADE_VULKAN)
		/*
		 * Match terrain SPIR-V shroud UV exactly (offset * scale into
		 * shroud_transform). World_XY Extract was sampling border/shrouded
		 * texels and painting roads solid black once ZFUNC became LEQUAL.
		 *
		 * Always MultiplicativeSprite: FOW factor is (1-a) from the mesh
		 * shader (black RGB + AlphaSprite left models undarkened).
		 */
		if (DX8Wrapper::Vulkan_Device_Active() && stage < 2) {
			D3DXMATRIX offsetMat;
			D3DXMatrixTranslation(&offsetMat, xoffset, yoffset, 0.0f);
			D3DXMATRIX scaleMat;
			D3DXMatrixScaling(&scaleMat, scaleX, scaleY, 1.0f);
			D3DXMATRIX shroudTransform = offsetMat * scaleMat;
			ww3d_vulkan::Set_Shroud_Transform(&shroudTransform._11);
			uint32_t meshShroudFlags = 1u << 21; /* FLAG_MESH_SHROUD */
			if (GlobalData_Fog_Of_War_Render_Enabled(TheGlobalData)) {
				meshShroudFlags |= (1u << 20); /* FLAG_TERRAIN_FOW_SHROUD */
			}
			ww3d_vulkan::Set_Extra_Draw_Flags(meshShroudFlags);
			DX8Wrapper::Set_Shader(ShaderClass::_PresetMultiplicativeSpriteShader);
...
void ShroudTextureShader::reset(void)
{
	...
	ww3d_vulkan::Clear_Extra_Draw_Flags();
#endif
}
...
#if defined(RENEGADE_VULKAN)
///Vulkan SPIR-V terrain shader — single-pass equivalent of terrain.pso.
class TerrainShaderVulkan : public W3DShaderInterface
{
public:
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		W3DShaderManager::applyVulkanTerrainPassMode(FALSE);
	}
#endif
...
#if defined(RENEGADE_VULKAN)
Int TerrainShaderVulkan::init(void)
{
	if (!DX8Wrapper::Vulkan_Device_Active()) {
		return FALSE;
	}
	/* Seed cloud scroll state via 2-stage helper, then own the W3DShaders slots
	 * so reset() clears SPIR-V terrain mode (2Stage::init registers itself). */
	if (!terrainShader2Stage.init()) {
		return FALSE;
	}
	W3DShaders[W3DShaderManager::ST_TERRAIN_BASE] = &terrainShaderVulkan;
	W3DShaders[W3DShaderManager::ST_TERRAIN_BASE_NOISE1] = &terrainShaderVulkan;
	W3DShaders[W3DShaderManager::ST_TERRAIN_BASE_NOISE2] = &terrainShaderVulkan;
	W3DShaders[W3DShaderManager::ST_TERRAIN_BASE_NOISE12] = &terrainShaderVulkan;
	W3DShaderManager::applyVulkanTerrainPassMode(FALSE);
	return TRUE;
}

void TerrainShaderVulkan::reset(void)
{
	ShaderClass::Invalidate();
	W3D_Bind_Shader_Stage_Texture(0, NULL);
	W3D_Bind_Shader_Stage_Texture(1, NULL);
	ww3d_vulkan::Texture_Stage_Bind_Null(2);
	ww3d_vulkan::Texture_Stage_Bind_Null(3);
	ww3d_vulkan::Clear_Terrain_Cloud_Params();
	ww3d_vulkan::Clear_Extra_Draw_Flags();
	ww3d_vulkan::Clear_Native_Texture_State();
	...
	ww3d_vulkan::Set_Shroud_Transform(identity);
}

Int TerrainShaderVulkan::set(Int pass)
{
	if (pass != 0) {
		return 0;
	}
	...
	W3D_Bind_Shader_Stage_Texture(0, W3DShaderManager::getShaderTexture(0));
	DX8Wrapper::Set_Texture(0, W3DShaderManager::getShaderTexture(0));

	/* Stage 1 is shroud only — never leave the terrain atlas bound here. */
	bool shroudBound = false;
	if (TheTerrainRenderObject != NULL) {
		W3DShroud *shroud = TheTerrainRenderObject->getShroud();
		if (shroud != NULL) {
			TextureClass *shroudTex = shroud->getShroudTexture();
			if (shroudTex != NULL) {
				W3D_Bind_Shader_Stage_Texture(1, shroudTex);
				DX8Wrapper::Set_Texture(1, shroudTex);
				shroudBound = true;

				/* The SPIR-V terrain shader computes shroud UV from world-space
				 * positions, so the projection matrix must be world->shroud-UV:
				 * offset into shroud space, then scale to texture units. */
				float xoffset = 0.0f;
				float yoffset = 0.0f;
				Real width = shroud->getCellWidth();
				Real height = shroud->getCellHeight();
				...
				D3DXMATRIX offset;
				D3DXMatrixTranslation(&offset, xoffset, yoffset, 0.0f);
				...
				D3DXMATRIX shroudTransform = offset * scale;
				ww3d_vulkan::Set_Shroud_Transform(&shroudTransform._11);
			}
		}
	}
	...
	/* Cloud (noise1) + lightmap (noise2) — bound as Vulkan stages 2/3 (beyond DX8 MAX_TEXTURE_STAGES). */
	{
		const W3DShaderManager::ShaderTypes cur = W3DShaderManager::getCurrentShader();
		const bool wantCloud =
			(cur == W3DShaderManager::ST_TERRAIN_BASE_NOISE1 ||
			 cur == W3DShaderManager::ST_TERRAIN_BASE_NOISE12);
		const bool wantNoise =
			(cur == W3DShaderManager::ST_TERRAIN_BASE_NOISE2 ||
			 cur == W3DShaderManager::ST_TERRAIN_BASE_NOISE12);

		uint32_t cloudFlags = 0;
		TextureClass *cloudTex = wantCloud ? W3DShaderManager::getShaderTexture(2) : NULL;
		TextureClass *noiseTex = wantNoise ? W3DShaderManager::getShaderTexture(3) : NULL;

		if (wantCloud && cloudTex != NULL) {
			ww3d_vulkan::Texture_Stage_Bind(cloudTex, 2);
			cloudFlags |= (1u << 16); /* FrameUBO::FLAG_TERRAIN_CLOUD */
		} else {
			ww3d_vulkan::Texture_Stage_Bind_Null(2);
		}
		if (wantNoise && noiseTex != NULL) {
			ww3d_vulkan::Texture_Stage_Bind(noiseTex, 3);
			cloudFlags |= (1u << 17); /* FrameUBO::FLAG_TERRAIN_NOISE */
		} else {
			ww3d_vulkan::Texture_Stage_Bind_Null(3);
		}
		if (GlobalData_Fog_Of_War_Render_Enabled(TheGlobalData)) {
			cloudFlags |= (1u << 20); /* FrameUBO::FLAG_TERRAIN_FOW_SHROUD */
		}

		float stretch = 1.0f / (63.0f * MAP_XY_FACTOR / 2.0f);
		/* Always advance cloud scroll time so NOISE2-only stretches do not jump. */
		D3DXMATRIX viewInv;
		D3DXMATRIX scratch;
		D3DXMatrixIdentity(&viewInv);
		terrainShader2Stage.updateNoise1(&scratch, &viewInv, true);
		float scrollX = wantCloud ? terrainShader2Stage.m_xOffset : 0.0f;
		float scrollY = wantCloud ? terrainShader2Stage.m_yOffset : 0.0f;
		ww3d_vulkan::Set_Terrain_Cloud_Params(scrollX, scrollY, stretch, cloudFlags);
		ww3d_vulkan::Set_Terrain_Cloud_Textures(cloudTex, noiseTex);

		static bool s_cloudDiagOnce = false;
		if (!s_cloudDiagOnce) {
			s_cloudDiagOnce = true;
			fprintf(stderr,
				"terrain_diag: cloud/noise set cur=%d wantCloud=%d wantNoise=%d "
				"cloud=%p noise=%p flags=0x%x scroll=(%.3f,%.3f) stretch=%.6f "
				"useCloudMap=%d useLightMap=%d shroud=%d\n",
				(int)cur, (int)wantCloud, (int)wantNoise,
				(void *)cloudTex, (void *)noiseTex, cloudFlags,
				scrollX, scrollY, stretch,
				TheGlobalData ? (int)TheGlobalData->m_useCloudMap : -1,
				TheGlobalData ? (int)TheGlobalData->m_useLightMap : -1,
				(int)shroudBound);
		}
	}

	if (W3D_Bilinear_Or_Trilinear_Terrain()) {
	...
	ww3d_vulkan::Terrain_Push_Spirv_Draw_State();
	return 0;
}

void W3DShaderManager::applyVulkanTerrainPassMode(Bool multipass)
{
	if (!DX8Wrapper::Vulkan_Device_Active()) {
		return;
	}
	(void)multipass;
	/* SPIR-V single-pass: base blend + shroud + optional cloud/noise modulate. */
...
#if defined(RENEGADE_VULKAN)
	/* W3DShaders[] points here after init(), so this reset runs for terrain.
	 * Must clear SPIR-V terrain mode or subsequent mesh draws use terrain.pso. */
	if (DX8Wrapper::Vulkan_Device_Active()) {
		ww3d_vulkan::Texture_Stage_Bind_Null(2);
		ww3d_vulkan::Texture_Stage_Bind_Null(3);
		ww3d_vulkan::Clear_Terrain_Cloud_Params();
...
{
#if defined(RENEGADE_VULKAN)
	/* Vulkan always uses SPIR-V pass 0; multipass mesh blend is not supported. */
	if (DX8Wrapper::Vulkan_Device_Active() && TheGlobalData != nullptr) {
		if (pass == 0) {
			return terrainShaderVulkan.set(0);
		}
		if (W3DShaderManager::getCurrentShader() ==
				W3DShaderManager::ST_TERRAIN_BASE) {
			return 0;
		}
		if (pass == 1) {
			/* Noise/cloud pass must use mesh pipeline — clear SPIR-V draw flag. */
			ww3d_vulkan::Clear_Terrain_Cloud_Params();
			ww3d_vulkan::Clear_Native_Texture_State();
			pass = 2;
		} else {
	...
	switch (pass)
	{
		case 0:
			W3D_Bind_Shader_Stage_Texture(0, W3DShaderManager::getShaderTexture(0));
#if defined(RENEGADE_VULKAN)
			ww3d_vulkan::Terrain_Push_Vulkan_Draw_State(0, false);
#endif
			DX8Wrapper::Set_DX8_Texture_Stage_State( 0, D3DTSS_ADDRESSU, D3DTADDRESS_CLAMP);
			DX8Wrapper::Set_DX8_Texture_Stage_State( 0, D3DTSS_ADDRESSV, D3DTADDRESS_CLAMP);
			...
			break;
		case 1:
			W3D_Bind_Shader_Stage_Texture(0, W3DShaderManager::getShaderTexture(1));
			DX8Wrapper::Set_DX8_Texture_Stage_State( 0, D3DTSS_ADDRESSU, D3DTADDRESS_CLAMP);
			DX8Wrapper::Set_DX8_Texture_Stage_State( 0, D3DTSS_ADDRESSV, D3DTADDRESS_CLAMP);
		...
		case 2:
			...
			{
				//setup cloud pass
				W3D_Bind_Shader_Stage_Texture(0, W3DShaderManager::getShaderTexture(2));
				...
				//setup noise pass
				W3D_Bind_Shader_Stage_Texture(1, W3DShaderManager::getShaderTexture(3));
			...
			{	//only 1 noise or cloud texture
				...
				{
					//setup noise pass
					W3D_Bind_Shader_Stage_Texture(0, W3DShaderManager::getShaderTexture(3));
...
#if defined(RENEGADE_VULKAN)
			if (DX8Wrapper::Vulkan_Device_Active()) {
				ShaderClass cloud_shader;
				DX8Wrapper::Get_Shader(cloud_shader);
				/*
				 * SRCBLEND_ONE_MINUS_SRC_ALPHA maps via srcBlendLUT[3] to
				 * D3DBLEND_DESTCOLOR (WW3D naming quirk) + DESTBLEND_ZERO.
				 */
				cloud_shader.Set_Src_Blend_Func(
					ShaderClass::SRCBLEND_ONE_MINUS_SRC_ALPHA);
				cloud_shader.Set_Dst_Blend_Func(ShaderClass::DSTBLEND_ZERO);
				cloud_shader.Set_Depth_Mask(ShaderClass::DEPTH_WRITE_DISABLE);
				DX8Wrapper::Set_Shader(cloud_shader);
			}
#endif
			break;
	}
...
Int TerrainShader8Stage::set(Int pass)
{
	if (pass == 0)
	{
		...
		W3D_Bind_Shader_Stage_Texture(0, W3DShaderManager::getShaderTexture(0));
		W3D_Bind_Shader_Stage_Texture(1, W3DShaderManager::getShaderTexture(1));
...
Int TerrainShaderPixelShader::set(Int pass)
{
	...
	{	
		...
		{	//full shader
			...
			W3D_Bind_Shader_Stage_Texture(2, W3DShaderManager::getShaderTexture(2));
			W3D_Bind_Shader_Stage_Texture(3, W3DShaderManager::getShaderTexture(3));
		...
		{	//single noise texture shader
			...
			{	//light map
				W3D_Bind_Shader_Stage_Texture(2, W3DShaderManager::getShaderTexture(3));
				terrainShader2Stage.updateNoise2(((D3DXMATRIX*)&curView),&inv);	//update curView with texture matrix
...
{
	W3D_Bind_Shader_Stage_Texture(2, NULL);	//release reference to any texture
	W3D_Bind_Shader_Stage_Texture(3, NULL);	//release reference to any texture
...
Int CloudTextureShader::set(Int stage)
{
	...
	W3D_Bind_Shader_Stage_Texture(stage, W3DShaderManager::getShaderTexture(stage));

#if defined(RENEGADE_VULKAN)
	/*
	 * Bridges/objects use mesh SPIR-V stage1 modulate. Ensure ShaderClass
	 * PostDetail matches D3DTOP_MODULATE so Native_Render_State_On_Shader
	 * does not wipe COLOROP after Apply. Use terrain-equivalent world XY UVs.
	 */
	if (DX8Wrapper::Vulkan_Device_Active() && stage == 1) {
		const float stretch = 1.0f / (63.0f * MAP_XY_FACTOR / 2.0f);
		ww3d_vulkan::Set_World_XY_Tex_Projection(
			1, stretch, terrainShader2Stage.m_xOffset, terrainShader2Stage.m_yOffset);
		ShaderClass cloud_shader;
		DX8Wrapper::Get_Shader(cloud_shader);
		cloud_shader.Set_Post_Detail_Color_Func(ShaderClass::DETAILCOLOR_SCALE);
		cloud_shader.Set_Post_Detail_Alpha_Func(ShaderClass::DETAILALPHA_SCALE);
		DX8Wrapper::Set_Shader(cloud_shader);
	}
#endif
...
void CloudTextureShader::reset(void)
{
	...
	W3D_Bind_Shader_Stage_Texture(m_stageOfSet, NULL);
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		ShaderClass cloud_shader;
		DX8Wrapper::Get_Shader(cloud_shader);
		cloud_shader.Set_Post_Detail_Color_Func(ShaderClass::DETAILCOLOR_DISABLE);
		cloud_shader.Set_Post_Detail_Alpha_Func(ShaderClass::DETAILALPHA_DISABLE);
		DX8Wrapper::Set_Shader(cloud_shader);
	}
#endif
}
...
#if defined(RENEGADE_VULKAN)
	/*
	 * Vulkan mesh SPIR-V cannot express D3D ALPHAREPLICATE + BLENDCURRENTALPHA.
	 * Pass 1 becomes: out = mix(1, lightmap, roadAlpha), blend ZERO/SRCCOLOR.
	 */
	if (DX8Wrapper::Vulkan_Device_Active() && pass == 1) {
		if (W3DShaderManager::getShaderTexture(0)) {
			W3D_Bind_Shader_Stage_Texture(0, W3DShaderManager::getShaderTexture(0));
		}
		W3D_Bind_Shader_Stage_Texture(1, W3DShaderManager::getShaderTexture(2));
		...
		const float stretch = 1.0f / (63.0f * MAP_XY_FACTOR / 2.0f);
		ww3d_vulkan::Set_World_XY_Tex_Projection(1, stretch, 0.0f, 0.0f);

		ww3d_vulkan::Set_Extra_Draw_Flags(1u << 18); /* FrameUBO::FLAG_ROAD_LIGHTMAP_MASK */
	...
	{	
		...
		{	//second texture unit will contain a noise pass
			...
			{	//full shader, apply noise 1 in pass 0.
				...
				terrainShader2Stage.updateNoise1(((D3DXMATRIX*)&curView),&inv, false);	//get texture projection matrix
				DX8Wrapper::_Set_DX8_Transform(D3DTS_TEXTURE1, curView);
				scrollX = terrainShader2Stage.m_xOffset;
				scrollY = terrainShader2Stage.m_yOffset;
			}
			else
			{	//single noise texture shader
				if (W3DShaderManager::getCurrentShader() == W3DShaderManager::ST_ROAD_BASE_NOISE1)
...
#if defined(RENEGADE_VULKAN)
			/* Force cloud UV + modulate in mesh SPIR-V (ignore PostDetail wipe). */
			if (DX8Wrapper::Vulkan_Device_Active()) {
				ww3d_vulkan::Set_World_XY_Tex_Projection(1, stretch, scrollX, scrollY);
				ww3d_vulkan::Set_Terrain_Cloud_Params(scrollX, scrollY, stretch, 0);
			}
#endif
		}
		else
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		ShaderClass road_shader = (pass == 0)
			? ShaderClass::_PresetAlphaShader
			: ShaderClass::_PresetMultiplicativeShader;
		/* Enable stage-1 modulate for cloud/lightmap on pass 0. */
		if (pass == 0 &&
				W3DShaderManager::getCurrentShader() >= W3DShaderManager::ST_ROAD_BASE_NOISE1) {
			road_shader.Set_Post_Detail_Color_Func(ShaderClass::DETAILCOLOR_SCALE);
			road_shader.Set_Post_Detail_Alpha_Func(ShaderClass::DETAILALPHA_SCALE);
		}
		DX8Wrapper::Set_Shader(road_shader);

		/*
		 * Apply shroud once in the base road color (pass 0). A separate
		 * multiplicative shroud pass stacks at overlapping tee/curve joins.
		 */
		if (pass == 0) {
			uint32_t flags = 0;
			if (W3DShaderManager::getCurrentShader() >= W3DShaderManager::ST_ROAD_BASE_NOISE1) {
				flags |= (1u << 19); /* FLAG_ROAD_CLOUD_MOD */
			}
			TextureClass *shroudTex = NULL;
			if (TheTerrainRenderObject != NULL && TheTerrainRenderObject->getShroud() != NULL) {
				shroudTex = TheTerrainRenderObject->getShroud()->getShroudTexture();
			}
			if (shroudTex != NULL) {
				W3DShroud *shroud = TheTerrainRenderObject->getShroud();
				float xoffset = 0.0f;
				...
				D3DXMATRIX shroudTransform = offset * scale;
				ww3d_vulkan::Set_Shroud_Transform(&shroudTransform._11);
				ww3d_vulkan::Texture_Stage_Bind(shroudTex, 2);
				flags |= (1u << 23); /* FLAG_MESH_INLINE_SHROUD */
				if (GlobalData_Fog_Of_War_Render_Enabled(TheGlobalData)) {
					flags |= (1u << 20); /* FLAG_TERRAIN_FOW_SHROUD */
				}
			} else {
				ww3d_vulkan::Texture_Stage_Bind_Null(2);
			}
			ww3d_vulkan::Set_Extra_Draw_Flags(flags);
		}
	}
#endif
...
void RoadShader2Stage::reset(void)
{
	...
	DX8Wrapper::Set_DX8_Texture_Stage_State( 1, D3DTSS_TEXCOORDINDEX, D3DTSS_TCI_PASSTHRU|1);
#if defined(RENEGADE_VULKAN)
	/* Drop World_XY + stage1 MODULATE left by cloud/lightmap road passes. */
	DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_COLOROP, D3DTOP_DISABLE);
	DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_ALPHAOP, D3DTOP_DISABLE);
	ww3d_vulkan::Texture_Stage_Bind_Null(2);
	ww3d_vulkan::Clear_Extra_Draw_Flags();
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		applyVulkanTerrainPassMode(FALSE);
	}
#endif
}
...
ChipsetType W3DShaderManager::getChipset( void )
{
	...
	/*	HRESULT res = */ d3d8Interface->GetAdapterIdentifier(0,D3DENUM_NO_WHQL_LEVEL,&did);
		...
		Int maxTextures=DX8Wrapper::Get_Current_Caps()->Get_Max_Textures_Per_Pass();
		Real pixelShaderVersion;

		char buf[256];

		//Convert version to Real
		sprintf(buf,"%d.%d",DX8Wrapper::Get_Current_Caps()->Get_Pixel_Shader_Majon_Version(),DX8Wrapper::Get_Current_Caps()->Get_Pixel_Shader_Minor_Version());
