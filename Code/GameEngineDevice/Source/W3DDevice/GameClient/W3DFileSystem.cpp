...
char const * GameFileClass::Set_Name( char const *filename )
{
	...
	// Models reference .tga; retail data stores .dds in Textures.big.
	if (fileType == FILE_TYPE_TGA) {
		char ddsPath[_MAX_PATH];
		strcpy(ddsPath, m_filePath);
		Int pathLen = (Int)strlen(ddsPath);
		if (pathLen >= 4 && stricmp(ddsPath + pathLen - 4, ".tga") == 0) {
			ddsPath[pathLen - 3] = 'd';
			ddsPath[pathLen - 2] = 'd';
			ddsPath[pathLen - 1] = 's';
			if (TheFileSystem->doesFileExist(ddsPath)) {
				strcpy(m_filePath, ddsPath);
				m_fileExists = TRUE;
			}
		}
	}
