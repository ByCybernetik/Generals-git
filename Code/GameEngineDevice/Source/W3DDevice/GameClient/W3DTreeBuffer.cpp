...
void W3DTreeBuffer::loadTreesInVertexAndIndexBuffers(RefRenderObjListIterator *pDynamicLightsIterator)
{
	...
	for (curTree=0; curTree<m_numTrees; curTree++) {
		...
		if (!doVertexLighting) {
			...
			Real treeB = shadeB;
#if defined(RENEGADE_VULKAN)
			/* Dim trees in fog/shroud like foggedLightEnv does for ghosts. */
			if (DX8Wrapper::Vulkan_Device_Active() &&
					TheTerrainRenderObject && TheTerrainRenderObject->getShroud() &&
					TheGlobalData && ThePartitionManager)
			{
				Int cx = 0;
				Int cy = 0;
				ThePartitionManager->worldToCell(loc.X, loc.Y, &cx, &cy);
				const W3DShroudLevel level =
					TheTerrainRenderObject->getShroud()->getShroudLevel(cx, cy);
				const Real clearA = (Real)TheGlobalData->m_clearAlpha;
				Real frac = (clearA > 0.0f) ? ((Real)level / clearA) : 1.0f;
				if (frac < 0.0f) {
					frac = 0.0f;
				} else if (frac > 1.0f) {
					frac = 1.0f;
...
#endif
			diffuse = doLighting(&loc, treeR, treeG, treeB, m_trees[curTree].bounds, NULL);
		}
