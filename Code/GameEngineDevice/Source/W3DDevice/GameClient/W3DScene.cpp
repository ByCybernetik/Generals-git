	...
	if (!drawableHidden)
	{
		...
		if (drawInfo)
		{
			...
			if (m_customPassMode == SCENE_PASS_DEFAULT)	
			{
				...
				/*
				 * FOW object darkening must follow foggedLightEnv (same as ghosts).
				 * The ST_SHROUD_TEXTURE multiply pass samples black RGB+alpha and
				 * was painting models solid black when approaching clear/fog borders.
				 */
				const Bool vulkanFow =
					DX8Wrapper::Vulkan_Device_Active() &&
					GlobalData_Fog_Of_War_Render_Enabled(TheGlobalData);
				if (vulkanFow &&
						(ss == OBJECTSHROUD_FOGGED || ss == OBJECTSHROUD_PARTIAL_CLEAR))
				{
					rinfo.light_environment = &m_foggedLightEnv;
					robj->Render(rinfo);
				}
				else if (ss <= OBJECTSHROUD_CLEAR)
					robj->Render(rinfo);
				else if (!vulkanFow)
				{
...
{
	/*
	 * Fog-of-war object dimming (ghosts / fogged props). Independent of
	 * m_disableFog, which only turns off atmospheric distance fog.
	 */
	Real foggedLightFrac = (Real)TheGlobalData->m_fogAlpha/(Real)TheGlobalData->m_clearAlpha;
	if (foggedLightFrac < 0.0f) {
		foggedLightFrac = 0.0f;
	} else if (foggedLightFrac > 1.0f) {
		foggedLightFrac = 1.0f;
	}
	Vector3 oldDiffuse;
