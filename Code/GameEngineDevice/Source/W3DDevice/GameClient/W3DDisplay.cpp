...
static Bool isSupportedDisplayResolution(const ResolutionDescClass &resolution)
{
...
*/
	...
	{
		if (isSupportedDisplayResolution(resolutions[res]))
		{	
			numResolutions++;
		}
	}
...
void Reset_D3D_Device(bool active)
{
	...
	/*
	 * Vulkan does not lose the logical device on minimize/alt-tab the way
	 * D3D8 did. Calling Set_Render_Device → Create_Device here re-inits the
	 * renderer (double pipelines/swapchain) and leaves skinned infantry /
	 * housecolor textures in a broken state until restart.
	 */
	(void)active;
	return;
#else
...
//============================================================================
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/*
	 * Renegade init.cpp: Wayland compositors keep the surface unmapped until the
	 * first swapchain present — without this only the taskbar entry appears.
	 */
	::ShowWindow(ApplicationHWnd, SW_SHOW);
	for (int splash_frame = 0; splash_frame < 3; ++splash_frame) {
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		TheGameLODManager->setStaticLODLevel(STATIC_GAME_LOD_HIGH);
#else
	...
	init3DScene();
#if defined(RENEGADE_VULKAN)
	/* SPIR-V single-pass terrain is the correct default on Vulkan. */
	if (DX8Wrapper::Vulkan_Device_Active() && TheWritableGlobalData != nullptr) {
		TheWritableGlobalData->m_multiPassTerrain = FALSE;
		TheWritableGlobalData->m_disableFog = TRUE;
		DX8Wrapper::Set_Debug_Disable_Fog(TRUE);
	}
#endif
...
AGAIN:
	...
	Bool pauseOrScriptFreeze = TheTacticalView->isTimeFrozen() && !TheTacticalView->isCameraMovementFinished();
	pauseOrScriptFreeze = pauseOrScriptFreeze || TheScriptEngine->isTimeFrozenDebug() || TheScriptEngine->isTimeFrozenScript();
	pauseOrScriptFreeze = pauseOrScriptFreeze || TheGameLogic->isGamePaused();

	/*
	 * sameLogicFrame: visual extras between logic ticks. Must NOT feed freezeTime for the
	 * legacy do-while (non-Linux) — that loop spins until camera finishes.
	 */
	static UnsignedInt lastFrame = ~0;
	const Bool sameLogicFrame = (lastFrame == TheGameClient->getFrame());
	lastFrame = TheGameClient->getFrame();

	Bool freezeTime = pauseOrScriptFreeze;

	/// @todo: I'm assuming the first view is our main 3D view.
	W3DView *primaryW3DView=(W3DView *)getFirstView();
	if (!freezeTime && !sameLogicFrame && TheScriptEngine->isTimeFast())
	{
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/*
	 * Visual WW3D clock: advance by wall time so mesh/UV animations stay smooth between
	 * 30 Hz logic ticks. Pause/script freeze still stops the clock.
	 */
	{
		static DWORD s_lastSyncWallMs = 0;
		const DWORD wallNow = timeGetTime();
		DWORD wallDelta = (s_lastSyncWallMs != 0) ? (wallNow - s_lastSyncWallMs) : (DWORD)TheW3DFrameLengthInMsec;
		s_lastSyncWallMs = wallNow;
		if (wallDelta < 1)
			wallDelta = 1;
		else if (wallDelta > 100)
			wallDelta = 100;
		if (!pauseOrScriptFreeze)
			syncTime += wallDelta;
	}
#else
	if (!freezeTime) 
	...
	do {
		
		{
			if(TheGlobalData->m_loadScreenRender != TRUE)
			{
				...
				{
					const Int remaining = (minTime - 1) - (Int)(now - prevTime);
					Sleep(remaining > 1 ? (DWORD)remaining : 1);
					now = timeGetTime();
				}
				prevTime = now;
			}
		...
		const Bool deviceReady =
			...
			(DX8Wrapper::_Get_D3D_Device8() != NULL &&
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
					{
						static UnsignedInt s_lastCineFrame = ~0;
						const UnsignedInt cineFrame = TheGameClient ? TheGameClient->getFrame() : 0;
						if (cineFrame != s_lastCineFrame) {
							s_lastCineFrame = cineFrame;
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/*
	 * Do not spin here until the camera finishes. That loop slept ~30ms/iter (~33 FPS)
	 * and blocked the outer 60 visual FPS path. Scripted camera advances on wall-clock
	 * each visual frame while GameLogic stays frozen for cameraModFreezeTime.
	 */
	} while (0);
#else
														...
														Int endX, Int endY, Color color, DrawImageMode mode)
{
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		...
		const float uSpan = uv_rect.Width();
		const float vSpan = uv_rect.Height();
		if (texSize->x > 1 && texSize->y > 1 &&
				(uSpan < 0.999f || vSpan < 0.999f || uv->lo.x > 0.001f || uv->lo.y > 0.001f)) {
			const float halfU = 0.5f / (float)texSize->x;
			const float halfV = 0.5f / (float)texSize->y;
			uv_rect.Left += halfU;
			uv_rect.Right -= halfU;
			uv_rect.Top += halfV;
			uv_rect.Bottom -= halfV;
		}
	}
#endif
...
#if defined(RENEGADE_VULKAN)
	/*
	 * Native Vulkan UI path: clip → resolve texture → Draw_Native_* → coalesce.
	 * Avoids Reset / Set_Texture / Add_Quad / Render per image (~450×/frame in Skirmish).
	 */
	if (DX8Wrapper::Vulkan_Device_Active() && m_2DRender != NULL) {
		static AsciiString s_texCacheName;
		static TextureClass *s_texCache = NULL;

		TextureClass *tex = NULL;
		Bool release_tex = FALSE;
		if (BitTest(image->getStatus(), IMAGE_STATUS_RAW_TEXTURE)) {
			tex = (TextureClass *)(image->getRawTextureData());
		} else {
			AsciiString fname = image->getFilename();
			if (s_texCache != NULL && fname == s_texCacheName) {
				tex = s_texCache;
			} else {
				tex = WW3DAssetManager::Get_Instance()->Get_Texture(
					fname.str(), TextureClass::MIP_LEVELS_1);
				if (s_texCache != NULL) {
					s_texCache->Release_Ref();
				}
				s_texCache = tex;
				s_texCacheName = fname;
			}
		}

		Render2DClass::NativeUIBlend blend = Render2DClass::NATIVE_UI_BLEND_ALPHA;
		...
		switch (mode) {
			...
			case DRAW_IMAGE_ADDITIVE:
				blend = Render2DClass::NATIVE_UI_BLEND_ADDITIVE;
				break;
			case DRAW_IMAGE_SOLID:
				blend = Render2DClass::NATIVE_UI_BLEND_SOLID;
				break;
		...
		if (BitTest(image->getStatus(), IMAGE_STATUS_ROTATED_90_CLOCKWISE)) {
			m_2DRender->Draw_Native_Two_Tris(
				tex,
				Vector2(screen_rect.Left, screen_rect.Top),
				...
				Vector2(screen_rect.Right, screen_rect.Bottom),
				Vector2(screen_rect.Right, screen_rect.Top),
				Vector2(screen_rect.Left, screen_rect.Bottom),
			...
			m_2DRender->Draw_Native_Quad(
				tex, screen_rect, uv_rect, (unsigned long)color,
				grayscale != FALSE, blend);
		}
...
void W3DDisplay::drawVideoBuffer( VideoBuffer *buffer, Int startX, Int startY, Int endX, Int endY )
{
	...
	if (vbuffer == NULL || vbuffer->texture() == NULL) {
		return;
	}
	...
	if (DX8Wrapper::Vulkan_Device_Active()) {
		m_2DRender->Draw_Native_Quad(
			vbuffer->texture(),
			RectClass( (float)startX, (float)startY, (float)endX, (float)endY ),
			vbuffer->Rect( 0, 0, 1, 1),
			0xFFFFFFFF,
			false,
			Render2DClass::NATIVE_UI_BLEND_SOLID);
		return;
	}
#endif
