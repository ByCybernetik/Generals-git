...
#if defined(RENEGADE_VULKAN)
/** Terrain renders first with light_environment=NULL ("self lit"). D3D8 reused
 * stale HW lights; Vulkan reads rs.Lights explicitly and needs terrain globals. */
static void Apply_Terrain_Vulkan_Lighting()
{
	if (!DX8Wrapper::Vulkan_Device_Active()) {
		return;
	}

	if (TheGlobalData->m_disableLighting) {
		DX8Wrapper::Set_DX8_Render_State(
			D3DRS_AMBIENT,
			DX8Wrapper::Convert_Color(Vector3(1.0f, 1.0f, 1.0f), 0.0f));
		for (Int i = 0; i < 4; ++i) {
			DX8Wrapper::Set_Light(i, NULL);
		}
		return;
	}

	Vector3 ambient(
		TheGlobalData->m_terrainAmbient[0].red,
	...
	const Int numLights = TheGlobalData->m_numGlobalLights > 4 ? 4 : TheGlobalData->m_numGlobalLights;
	for (Int i = 0; i < numLights; ++i) {
		D3DLIGHT8 light;
		memset(&light, 0, sizeof(light));
		light.Type = D3DLIGHT_DIRECTIONAL;
		light.Diffuse.r = TheGlobalData->m_terrainDiffuse[i].red;
		light.Diffuse.g = TheGlobalData->m_terrainDiffuse[i].green;
		light.Diffuse.b = TheGlobalData->m_terrainDiffuse[i].blue;
		light.Diffuse.a = 1.0f;
		Vector3 dir(
			TheGlobalData->m_terrainLightPos[i].x,
			TheGlobalData->m_terrainLightPos[i].y,
			TheGlobalData->m_terrainLightPos[i].z);
		if (dir.Length() > 1e-6f) {
			dir.Normalize();
		} else {
		...
		DX8Wrapper::Set_Light(i, &light);
	}
	for (Int i = numLights; i < 4; ++i) {
		DX8Wrapper::Set_Light(i, NULL);
	}
}
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		/* Terrain SPIR-V mode forces opaque terrain_shader draws; scorches need alpha. */
		...
		DX8Wrapper::Vulkan_Flush_Pending_Draws();
		return;
	}
#endif
...
HeightMapRenderObjClass::HeightMapRenderObjClass(void)
{
	...
	clearAllScorches();
#endif
	if (GlobalData_Shroud_Render_Enabled(TheGlobalData))
...
static Bool check(const FrustumClass & frustum, WorldHeightMap *pMap, Int x, Int y) 
{
	...
	/*
	 * Accept INSIDE and OVERLAPPED. Trees use Cull_Sphere (visible unless OUTSIDE);
	 * requiring INSIDE only dropped ground samples at shallow camera angles and
	 * let the 129x129 draw window slide away from the visible map patch.
	 */
	const CollisionMath::OverlapType overlap = CollisionMath::Overlap_Test(frustum,loc);
	if (overlap == CollisionMath::INSIDE || overlap == CollisionMath::OVERLAPPED) {
...
//	float aspect = camera->Get_Aspect_Ratio();
	...
	for (i=0; i<2; i++) {
		for (j=0; j<2; j++) {
			...
			/*
			 * Intersect the view-corner ray with the minimum terrain height plane.
			 * Looking up / near-horizon often yields t<=0 (hit behind camera) or a
			 * near-zero dir.Z (Inf/NaN). Those used to expand the search box into
			 * garbage and slide the draw window off the visible terrain.
			 */
			const float dirZ = rayDirectionPt.Z - rayLocation.Z;
			if (WWMath::Fabs(dirZ) < 1.0e-4f) {
				continue;
			}
			const float tHit = (intersectionZ - rayLocation.Z) / dirZ;
			if (tHit <= 0.0f || !WWMath::Is_Valid_Float(tHit)) {
				continue;
			}
			x = rayLocation.X + tHit * (rayDirectionPt.X - rayLocation.X);
			y = rayLocation.Y + tHit * (rayDirectionPt.Y - rayLocation.Y);
			if (!WWMath::Is_Valid_Float(x) || !WWMath::Is_Valid_Float(y)) {
				continue;
			}
			gotGroundHit = true;
			if (x<minX) minX = x;
	...
	/* Always include the tactical look-at point so the draw window stays under the camera. */
	if (TheTacticalView) {
		Coord3D lookAt;
		TheTacticalView->getPosition(&lookAt);
		gotGroundHit = true;
		if (lookAt.x < minX) minX = lookAt.x;
		if (lookAt.x > maxX) maxX = lookAt.x;
		if (lookAt.y < minY) minY = lookAt.y;
		if (lookAt.y > maxY) maxY = lookAt.y;
	}

	if (!gotGroundHit) {
		/* No usable ground hits — keep the current draw origin rather than jumping. */
		m_updating = false;
		return;
	}
	...
	if (m_map) {
		const Bool haveVis = (visMaxX >= visMinX && visMaxY >= visMinY);
		Int newOrgX;
		Int newOrgY;
		if (!haveVis) {
			/*
			 * calcVis found nothing (common when looking up). Center on look-at /
			 * camera XY instead of (0+extent)/2 which teleports the window away.
			 */
			Real focusX = camera_location.X;
			Real focusY = camera_location.Y;
			if (TheTacticalView) {
				Coord3D lookAt;
				TheTacticalView->getPosition(&lookAt);
				focusX = lookAt.x;
			...
			newOrgX = (Int)(focusX / MAP_XY_FACTOR + m_map->getBorderSize() - m_x / 2.0f);
			newOrgY = (Int)(focusY / MAP_XY_FACTOR + m_map->getBorderSize() - m_y / 2.0f);
		} else {
			...
			if (TheTacticalView && TheTacticalView->getFieldOfView() != 0) {
				newOrgX = (visMaxX+visMinX)/2-m_x/2.0;
				newOrgY = (visMaxY+visMinY)/2-m_y/2.0;
			}
		}
...
#pragma MESSAGE("*** WARNING *** DOING DO_UNIT_TIMINGS!!!!")
...
#if defined(RENEGADE_VULKAN)
	/* Vulkan pipeline does not implement the alpha-mask COLORWRITEENABLE or
	 * D3DTSS_TCI_CAMERASPACEPOSITION paths used here. Fall through to the
	 * standard terrain pass so the landscape still draws. */
	if (DX8Wrapper::Vulkan_Device_Active()) {
		doAlphaMaskOrWireFrame = FALSE;
	}
#endif
	if (doAlphaMaskOrWireFrame)
	{
	...
	{
#if defined(RENEGADE_VULKAN)
		/* Commit terrain tiles before alpha overlays (roads, shorelines). */
		if (DX8Wrapper::Vulkan_Device_Active()) {
			DX8Wrapper::Vulkan_Flush_Pending_Draws();
		}
#endif
	...
	if (m_shroud && rinfo.Additional_Pass_Count())
	{
		...
		if (DX8Wrapper::Vulkan_Device_Active()) {
			/*
			 * The SPIR-V terrain shader already applies the shroud texture in the
			 * base terrain pass. The legacy ST_SHROUD_TEXTURE overlay pass relies
			 * on D3DTSS_TCI_CAMERASPACEPOSITION, which is not implemented for the
			 * generic Vulkan mesh pipeline and would just darken/blot the terrain.
			 */
		} else
#endif
		{
