	...
	if (is_temp && length <= MAX_TEMP_LEN && ReservedMask!=ALL_TEMP_STRINGS_USED_MASK) {
		...
		for (int index = 0; index < MAX_TEMP_STRING; index ++, mask<<=1) {
			...
			if (!(ReservedMask&mask)) {
				...
				//
				const uintptr_t pool_begin = reinterpret_cast<uintptr_t>(m_TempStrings);
				const uintptr_t pool_end = pool_begin + sizeof(m_TempStrings);
				uintptr_t temp_string = pool_begin
					+ static_cast<uintptr_t>(index) * MAX_TEMP_BYTES
					+ sizeof(_HEADER);

				if (temp_string < pool_begin || temp_string + MAX_TEMP_BYTES > pool_end) {
					string = NULL;
					ReservedMask &= ~mask;
				} else {
...
{
	if (m_Buffer != NULL && m_Buffer != m_EmptyString && m_Buffer != &m_NullChar) {

		const uintptr_t pool_begin = reinterpret_cast<uintptr_t>(m_TempStrings);
		const uintptr_t pool_end = pool_begin + sizeof(m_TempStrings);
		uintptr_t buffer_base = reinterpret_cast<uintptr_t>(m_Buffer) - sizeof(StringClass::_HEADER);

		if (buffer_base >= pool_begin && buffer_base < pool_end) {
			//
			...
			unsigned index = static_cast<unsigned>((buffer_base - pool_begin) / MAX_TEMP_BYTES);
			if (index < MAX_TEMP_STRING) {
			...
			if (header->allocated_length <= 0 || header->allocated_length > 65536) {
