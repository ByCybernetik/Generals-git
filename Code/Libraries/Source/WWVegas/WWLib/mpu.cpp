...
{
#if defined(__GNUC__)
	unsigned long long stamp = __rdtsc();
	high = (unsigned long)(stamp >> 32);
	return (unsigned long)(stamp & 0xffffffffu);
#else
	int h;
	int l;
	__asm {
...
// DWORD	current = 0;				// Elapsed time during loop
	...
	*/
	...
	} while ( (tries < 3 ) || (tries < 20) && ((labs((long)(3 * freq -total)) > 3*TOLERANCE )|| (labs((long)(3 * freq2-total)) > 3*TOLERANCE )|| (labs((long)(3 * freq3-total)) > 3*TOLERANCE )));
