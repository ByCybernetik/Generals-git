...
WideStringClass::Get_Allocated_Length (void) const
{
	...
	//
	if (m_Buffer != NULL && m_Buffer != &m_NullChar && m_Buffer != m_EmptyString) {		
...
WideStringClass::Convert_From(const char * text)
{
	...
	if (text == NULL || text[0] == 0) {
		Uninitialised_Grow(1);
		m_Buffer[0] = 0;
		return;
	}
	int need = generals_utf8_to_utf16(text, -1, NULL, 0, 1, NULL);
	if (need <= 0) {
		Uninitialised_Grow(1);
		m_Buffer[0] = 0;
		return;
	}
	Uninitialised_Grow(need);
	generals_utf8_to_utf16(
		text, -1, reinterpret_cast<uint16_t *>(m_Buffer), need, 1, NULL);
#else
	int len = ::strlen ( text );
	Uninitialised_Grow ( len + 1 );
	if ( len != 0 ) {
...
{
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	int need = generals_utf16_to_utf8(
		reinterpret_cast<const uint16_t *>(m_Buffer), -1, NULL, 0, 1, NULL);
	if (need <= 0) {
		string = "";
		return;
	}
	char *dest_buffer = string.Get_Buffer(need);
	generals_utf16_to_utf8(
		reinterpret_cast<const uint16_t *>(m_Buffer), -1, dest_buffer, need, 1, NULL);
#else
	//
