...
/*
** On-disk MIX1 layout uses 32-bit little-endian fields.
*/
typedef struct
{
	char	signature[4];
...
MixFileFactoryClass::Flush_Changes (void)
{
	...
	if (Get_Temp_Filename (path, full_path)) {
		...
		//
		for (int add_idx = 0; add_idx < PendingAddFileList.Count (); add_idx ++) {
			new_mix_file.Add_File (PendingAddFileList[add_idx].FullPath, PendingAddFileList[add_idx].Filename);
		}
	}
