...
bool
Render2DSentenceClass::Set_Wrapping_Width (float width)
{
	if (WrapWidth == width) {
		return false;
	}
...
bool
Render2DSentenceClass::Set_Use_Hard_Word_Wrap (bool onoff)
{
	if (UseHardWordWrap == onoff) {
		return false;
	}
	UseHardWordWrap = onoff;
	return true;
}

void
Render2DSentenceClass::Set_Hot_Key_Parse (bool onoff)
{
	HotKeyParse = onoff;
}

bool
Render2DSentenceClass::Set_Word_Wrap_Centered (bool onoff)
{
	if (WordWrapCentered == onoff) {
		return false;
	}
	WordWrapCentered = onoff;
	return true;
}
	...
	for (int index = 0; index < Renderers.Count (); index ++) {
		...
		if (DX8Wrapper::Vulkan_Device_Active()) {
			const ww3d_vulkan::VkCpuSurface *cpu =
				static_cast<const ww3d_vulkan::VkCpuSurface *>(curr_surface->Peek_Vulkan_Cpu_Surface());
			if (cpu != NULL) {
				...
				ww3d_vulkan::Apply_Texture_From_Cpu_Surface(new_texture, cpu);
