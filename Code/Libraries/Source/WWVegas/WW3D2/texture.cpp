...
#if defined(RENEGADE_VULKAN)
	if (Vulkan_Procedural_Surface != nullptr) {
		SurfaceClass *proc_surface =
			static_cast<SurfaceClass *>(Vulkan_Procedural_Surface);
		Vulkan_Procedural_Surface = nullptr;
		REF_PTR_RELEASE(proc_surface);
	}
	if (Vulkan_Texture != nullptr) {
		if (Vulkan_Texture_Owned) {
			ww3d_vulkan::Retire_Owned_Vulkan_Texture(
				static_cast<ww3d_vulkan::VkTexture *>(Vulkan_Texture));
		}
		Vulkan_Texture = nullptr;
		Vulkan_Texture_Owned = false;
	}
#endif
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		return ww3d_vulkan::Get_Texture_Mip_Level_Count(this);
	}
#endif
	if (!D3DTexture) {
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		if (ww3d_vulkan::Get_Texture_Level_Description(this, surface_desc, level)) {
			return;
		}
		surface_desc.Width = 256;
		surface_desc.Height = 256;
		surface_desc.Format = Get_Texture_Format();
		if (surface_desc.Format == WW3D_FORMAT_UNKNOWN) {
...
#if defined(RENEGADE_VULKAN)
	if (DX8Wrapper::Vulkan_Device_Active()) {
		return ww3d_vulkan::Create_Texture_Surface_Level(this, level);
	}
#endif
...
void TextureClass::Apply(unsigned int stage)
{
	...
	/*
	 * Filter/address TSS must be recorded before Vulkan Texture_Stage_Bind —
	 * that path builds VkSampler from g_stage. Applying filters after Bind left
	 * POINT/NONE (Native_Render_State_Reset defaults) on the sampler and caused
	 * close-up sparkle on DXT meshes (bunker camo, dirt pads, etc.).
	 */
	const unsigned min_f = _MinTextureFilters[TextureMinFilter];
	const unsigned mag_f = _MagTextureFilters[TextureMagFilter];
	const unsigned mip_f = _MipMapFilters[MipMapFilter];
	DX8Wrapper::Set_DX8_Texture_Stage_State(stage,D3DTSS_MINFILTER,min_f);
	DX8Wrapper::Set_DX8_Texture_Stage_State(stage,D3DTSS_MAGFILTER,mag_f);
	DX8Wrapper::Set_DX8_Texture_Stage_State(stage,D3DTSS_MIPFILTER,mip_f);
...
void TextureClass::_Init_Filters()
{
	...
	/* Caps stubs (e.g. pre-fix Vulkan) may leave BEST at 0 == D3DTEXF_NONE. */
	if (_MinTextureFilters[FILTER_TYPE_BEST] == 0) {
		_MinTextureFilters[FILTER_TYPE_BEST] = D3DTEXF_LINEAR;
	}
	if (_MagTextureFilters[FILTER_TYPE_BEST] == 0) {
		_MagTextureFilters[FILTER_TYPE_BEST] = D3DTEXF_LINEAR;
	}
	if (_MipMapFilters[FILTER_TYPE_BEST] == 0) {
		_MipMapFilters[FILTER_TYPE_BEST] = D3DTEXF_LINEAR;
	}

#if defined(RENEGADE_VULKAN)
	/*
	 * Prefer anisotropic (DX8 BEST) when caps advertise it. Only upgrade
	 * NONE/POINT — never demote ANISOTROPIC down to LINEAR.
	 */
	if (DX8Wrapper::Vulkan_Device_Active()) {
		if (_MinTextureFilters[FILTER_TYPE_BEST] <= D3DTEXF_POINT) {
			_MinTextureFilters[FILTER_TYPE_BEST] = D3DTEXF_LINEAR;
		}
		if (_MagTextureFilters[FILTER_TYPE_BEST] <= D3DTEXF_POINT) {
			_MagTextureFilters[FILTER_TYPE_BEST] = D3DTEXF_LINEAR;
		}
		if (_MipMapFilters[FILTER_TYPE_BEST] <= D3DTEXF_POINT) {
			_MipMapFilters[FILTER_TYPE_BEST] = D3DTEXF_LINEAR;
		}
		if (_MinTextureFilters[FILTER_TYPE_FAST] <= D3DTEXF_POINT) {
			_MinTextureFilters[FILTER_TYPE_FAST] = D3DTEXF_LINEAR;
		}
		if (_MagTextureFilters[FILTER_TYPE_FAST] <= D3DTEXF_POINT) {
			_MagTextureFilters[FILTER_TYPE_FAST] = D3DTEXF_LINEAR;
		}
		if (_MipMapFilters[FILTER_TYPE_FAST] <= D3DTEXF_POINT) {
			_MipMapFilters[FILTER_TYPE_FAST] = D3DTEXF_LINEAR;
		}
	}
#endif
