...
public:
	...
	/*
	 * Fast UI path: one textured quad/tri pair straight to Vulkan 2D coalesce.
	 * Skips Reset / DynamicVector Add_* / Render() overhead used by drawImage.
	 */
	enum NativeUIBlend {
		NATIVE_UI_BLEND_ALPHA = 0,
		NATIVE_UI_BLEND_ADDITIVE = 1,
		NATIVE_UI_BLEND_SOLID = 2
	};
	void Draw_Native_Quad(
		TextureClass *tex,
	...
	void	Add_Line( const Vector2 & a, const Vector2 & b, float width, unsigned long color1, unsigned long color2 );
