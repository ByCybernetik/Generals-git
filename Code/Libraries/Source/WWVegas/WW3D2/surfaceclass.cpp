...
void SurfaceClass::DrawPixel(const unsigned int x,const unsigned int y, unsigned int color)
{
	...
	if (Vulkan_Cpu_Surface != nullptr) {
		SurfaceDescription sd;
		Get_Description(sd);
		const unsigned int size = PixelSize(sd);
		ww3d_vulkan::VkCpuSurface *cpu =
			static_cast<ww3d_vulkan::VkCpuSurface *>(Vulkan_Cpu_Surface);
		int pitch = 0;
		unsigned char *base = static_cast<unsigned char *>(cpu->Lock(&pitch));
		unsigned char *pixel = base + y * (unsigned)pitch + x * size;
		switch (size) {
		case 1:
			*pixel = (unsigned char)(color & 0xFF);
			break;
		case 2:
			*reinterpret_cast<unsigned short *>(pixel) = (unsigned short)(color & 0xFFFF);
			break;
		case 3:
			...
			break;
		case 4:
			*reinterpret_cast<unsigned int *>(pixel) = color;
			break;
		}
		cpu->Unlock();
		return;
	...
	case 2:
		*sptr=(unsigned short) (color & 0xFFFF);
		break;
	case 3:
		cptr[0] = (unsigned char)(color & 0xFF);
		cptr[1] = (unsigned char)((color >> 8) & 0xFF);
		cptr[2] = (unsigned char)((color >> 16) & 0xFF);
		break;
	case 4:
		*lptr=color;
		break;
...
{ 
	...
	{		
		...
		case 3:
			*cptr++ = (unsigned char)(color & 0xFF);
			*cptr++ = (unsigned char)((color >> 8) & 0xFF);
			*cptr++ = (unsigned char)((color >> 16) & 0xFF);
			break;
		case 4:
			*lptr++=color;
			break;
