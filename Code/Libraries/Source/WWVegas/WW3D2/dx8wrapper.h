...
public:
	...
	*/
	...
	static WWINLINE unsigned Get_DX8_Render_State(D3DRENDERSTATETYPE state) { return RenderStates[state]; }
	...
	*/
	...
	static void						Set_Debug_Disable_Fog(bool disable);
	...
	*/
	...
	static void Set_Vulkan_Debug_Disable_Lighting(bool disable);
	static bool Vulkan_Debug_Disable_Lighting();
	...
	static void Vulkan_Flush_Pending_Draws();
	...
	static WW3DFormat BackBufferFormat;
	static WW3DFormat getBackBufferFormat() { return BackBufferFormat; }

	static TextureClass *Create_Render_Target(int width, int height)
	{
		return Create_Render_Target(width, height, WW3D_FORMAT_UNKNOWN);
	}

	static TextureClass *Create_Render_Target(int width, int height, bool withAlpha)
	{
		return Create_Render_Target(width, height, withAlpha ? WW3D_FORMAT_A8R8G8B8 : WW3D_FORMAT_UNKNOWN);
	}

	static void Invalidate_Cached_Render_States(void);
#if defined(RENEGADE_VULKAN)
	/** Poison TSS cache so the next Set_* actually reaches Native_Render_State. */
	static void Invalidate_Texture_Stage_State_Cache(unsigned stage, unsigned tss);
#endif
...
protected:
...
#endif
	static void Add_Common_Render_Resolutions(RenderDeviceDescClass &desc);
	...
	** Protected Member Variables
	...
	static DX8_CleanupHook*			CleanupHook;
...
WWINLINE int DX8Wrapper::Get_Effective_ZBias()
{
	...
	// Invalidate_Cached_Render_States() stamps every RenderStates[] slot with
	// the 0x12345678 sentinel to force Set_DX8_Render_State() past its
	// early-out.  Treat the sentinel as "unset" here; otherwise it would
	// clamp to the maximum ZBIAS of 15 and leak a non-zero depth bias into
	// every subsequent draw after the shadow pass invalidates the cache.
	if (rs_zbias != 0x12345678u && rs_zbias > (unsigned)effective_zbias) {
...
// Generals-specific hook for DX8 device reset (not in base Renegade WW3D2).
class DX8_CleanupHook
{
public:
