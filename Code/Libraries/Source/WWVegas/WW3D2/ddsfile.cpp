	...
	Format(WW3D_FORMAT_UNKNOWN)
{
	...
	uint8_t desc_bytes[LEGACY_DDSURFACEDESC2_BYTES];
	unsigned read_bytes = file->Read(desc_bytes, LEGACY_DDSURFACEDESC2_BYTES);
	const LegacyDdsHeaderFields parsed =
		Legacy_Dds_Parse_Header(desc_bytes, read_bytes);
	if (read_bytes != LEGACY_DDSURFACEDESC2_BYTES || !parsed.valid) {
		return;
	}
	memset(&SurfaceDesc, 0, sizeof(SurfaceDesc));
	SurfaceDesc.Size = LEGACY_DDSURFACEDESC2_BYTES;
	SurfaceDesc.Height = parsed.height;
	SurfaceDesc.Width = parsed.width;
	SurfaceDesc.MipMapCount = parsed.mip_map_count;
	SurfaceDesc.PixelFormat.FourCC = parsed.four_cc;
#else
