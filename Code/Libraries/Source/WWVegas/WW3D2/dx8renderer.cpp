...
void DX8SkinFVFCategoryContainer::Add_Visible_Skin(MeshClass * mesh) 
{
	...
	{
		WWASSERT_PRINT(0, "Mesh is already a visible skin (duplicate add)\n");
		return;
	}
...
void DX8TextureCategoryClass::Render(void)
{
	...
	if (!WW3D::Expose_Prelit()) {
		...
		/*
		 * Commit prior category draws before rebinding stage textures. Pending
		 * draws capture VkTexture*; destroying/replacing a texture mid-batch
		 * (or failing a bind and leaving a stale stage0) made nearby props
		 * (ptpalm/ptdogwod) appear on bunkers until garrison redraw shuffled
		 * the category order.
		 */
...
{
	/*
	 * Device reset / accidental second Create_Device must not replace the
	 * skin list: registered infantry polygon renderers live on the old list.
	 */
	if (texture_category_container_list_skin == NULL) {
