...
void GameEngine::setFramesPerSecondLimit( Int fps )
{
	...
	m_maxFPS = (fps > 0) ? fps : DEFAULT_MAX_FPS;
}
...
void GameEngine::init( int argc, char *argv[] )
{
...
#if !defined(GENERALS_DISABLE_AUDIO) && !defined(GENERALS_LINUX)
	...
	//			TheShell->hideShell();
		...
		{
			/* findMap() handles Linux '/' vs '\' cache keys; raw find() does not. */
			if (TheMapCache->findMap(TheGlobalData->m_shellMapName) == NULL)
			{
				TheWritableGlobalData->m_shellMapOn = FALSE;
			}
		}
	...
	catch (ErrorCode ec)
	{
		...
		{
			char buf[256];
			snprintf(buf, sizeof(buf), "Uncaught ErrorCode %d during initialization.", (int)ec);
			RELEASE_CRASH(buf);
		}
	}
...
{ 
	USE_PERF_TIMER(GameEngine_update)
	{
...

		{
			...
			TheAudio->UPDATE();
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
			FRAME_PROFILE_MARK("client");
#endif
			TheGameClient->UPDATE();
			TheMessageStream->propagateMessages();

#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
			FRAME_PROFILE_MARK("network");
#endif
			if (TheNetwork != NULL)
			{
				TheNetwork->UPDATE();
...
void GameEngine::execute( void )
{
	...
	FrameProfile_Init();
	/*
	 * Logic defaults to LOGICFRAMES_PER_SECOND; map SET_FPS_LIMIT may lower it to 20/25
	 * for cinematic timing. FramesPerSecondLimit / m_maxFPS only caps visual frames.
	 */
	DWORD logicAccumMs = 0;
	DWORD lastWallMs = timeGetTime();
	const Int maxLogicCatchUp = 3;
#endif
	...
	while( !m_quitting )
	{
...

		{
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
			{
				...
				DWORD wallDelta = wallNow - lastWallMs;
				lastWallMs = wallNow;
				if (wallDelta > 250)
					wallDelta = 250; /* avoid huge catch-up after hitch */
				logicAccumMs += wallDelta;

				/* Map cinematic SET_FPS_LIMIT → logic Hz; visual stays at m_maxFPS (e.g. 60). */
				DWORD logicStepMs = (DWORD)(MSEC_PER_LOGICFRAME_REAL + 0.5f); /* ~33 → 30 Hz */
				{
					const Int mapHz = getMapLogicHz();
					if (mapHz > 0)
					{
						logicStepMs = (DWORD)((1000.0f / (Real)mapHz) + 0.5f);
						if (logicStepMs < 1)
							logicStepMs = 1;
					}
				}
				...
				Int logicTicks = 0;
				const Bool timeFast = TheScriptEngine && TheScriptEngine->isTimeFast();
				const Bool timeMult = TheTacticalView && TheTacticalView->getTimeMultiplier() > 1;
				const Bool camFreeze = TheTacticalView
					&& TheTacticalView->isTimeFrozen()
					&& !TheTacticalView->isCameraMovementFinished();
				const Bool scriptFreeze = TheScriptEngine
					&& (TheScriptEngine->isTimeFrozenDebug() || TheScriptEngine->isTimeFrozenScript());
				const Bool logicFrozen = (TheGameLogic && TheGameLogic->isGamePaused())
					|| scriptFreeze || camFreeze;

				if (logicAccumMs >= logicStepMs && TheGameClient)
					TheGameClient->beginVisualInterpLogicBatch();

				while (logicAccumMs >= logicStepMs && logicTicks < maxLogicCatchUp)
				{
					try
					{
						update();
					...
					logicAccumMs -= logicStepMs;
					++logicTicks;
					FrameProfile_NoteLogicTick();
				}
				if (logicAccumMs > logicStepMs * (DWORD)maxLogicCatchUp)
					logicAccumMs = logicStepMs * (DWORD)maxLogicCatchUp;

				/* Fast-forward modes: drain accumulator by running logic every outer loop. */
				if ((timeFast || timeMult) && logicTicks == 0)
				{
					try { update(); }
					catch (...) { RELEASE_CRASH(("Uncaught Exception in GameEngine::update")); }
					logicAccumMs = 0;
					FrameProfile_NoteLogicTick();
				}

				Real visualDtMs = (Real)wallDelta;
				if (visualDtMs < 1.0f)
					visualDtMs = 1.0f;
				else if (visualDtMs > 100.0f)
					visualDtMs = 100.0f;

				Real interpAlpha = 1.0f;
				if (!logicFrozen && !timeFast && !timeMult && logicStepMs > 0)
				{
					interpAlpha = (Real)logicAccumMs / (Real)logicStepMs;
					if (interpAlpha < 0.0f)
						interpAlpha = 0.0f;
					else if (interpAlpha > 1.0f)
						interpAlpha = 1.0f;
				}
				if (TheGameClient)
				{
					TheGameClient->setVisualInterpAlpha(interpAlpha);
					TheGameClient->renderVisualFrame(visualDtMs);
				}
				FrameProfile_NoteVisualFrame();
			}

			{
				DWORD waitStart = timeGetTime();
				double fpsWaitMs = 0.0;

				if (TheTacticalView->getTimeMultiplier()<=1 && !TheScriptEngine->isTimeFast())
				{
					if (TheGlobalData->m_useFpsLimit && m_maxFPS > 0) {
						DWORD now = timeGetTime();
						DWORD limit = (1000u / (DWORD)m_maxFPS);
						if (limit > 1) {
							limit -= 1;
						}
						while ((now - prevTime) < limit) {
							Platform_Pump_Events();
							const DWORD remaining = limit - (now - prevTime);
				...
				fpsWaitMs = (double)(timeGetTime() - waitStart);
				FRAME_PROFILE_END(fpsWaitMs);
			}
#else
			{
				try 
