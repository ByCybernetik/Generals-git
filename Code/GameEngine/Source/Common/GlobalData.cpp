...
#include "GameClient/TerrainLODNameTables.h"
#include "GameLogic/Module/BodyDamageNameTables.h"
...
*/
	...
	{ "DisableLighting",					INI::parseBool,				NULL,			offsetof( GlobalData, m_disableLighting ) },
	{ "DisableFog",							INI::parseBool,				NULL,			offsetof( GlobalData, m_disableFog ) },
...
//	m_inGame = FALSE;	
	...
	m_disableFog = FALSE;
	m_renderDebugDisableShroud = FALSE;
	m_renderDebugDisableFogOfWar = FALSE;
...
void GlobalData::parseGameDataDefinition( INI* ini )
{
	...
	char temp[_MAX_PATH];
	Bool gotUserDataPath = FALSE;

#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	{
		const char *config_home = getenv("XDG_CONFIG_HOME");
		if (config_home != NULL && config_home[0] != '\0') {
			strncpy(temp, config_home, sizeof(temp) - 1);
			temp[sizeof(temp) - 1] = '\0';
			gotUserDataPath = TRUE;
		} else {
			const char *home = getenv("HOME");
			if (home != NULL && home[0] != '\0') {
				snprintf(temp, sizeof(temp), "%s/.config", home);
				gotUserDataPath = TRUE;
			}
		}
	}
#else
	gotUserDataPath = ::SHGetSpecialFolderPath(NULL, temp, CSIDL_PERSONAL, true);
#endif
	...
	if (gotUserDataPath)
	{
		...
		if (len > 0 && temp[len - 1] != sep && temp[len - 1] != '\\' && temp[len - 1] != '/') {
			strncat(temp, &sep, 1);
		}

		char leaf[_MAX_PATH];
		strncpy(leaf, TheWritableGlobalData->m_userDataLeafName.str(), sizeof(leaf) - 1);
		leaf[sizeof(leaf) - 1] = '\0';
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		if (leaf[0] == '\0') {
			strncpy(leaf, "Command and Conquer Generals Data/", sizeof(leaf) - 1);
			leaf[sizeof(leaf) - 1] = '\0';
		}
		for (char *p = leaf; *p; ++p) {
			if (*p == '\\') {
...
#endif
		strncat(temp, leaf, sizeof(temp) - strlen(temp) - 1);
		len = strlen(temp);
		if (len > 0 && temp[len - 1] != sep && temp[len - 1] != '\\' && temp[len - 1] != '/') {
			strncat(temp, &sep, 1);
		}
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		{
			static int user_data_path_log = 0;
			if (user_data_path_log < 4) {
				++user_data_path_log;
			}
		}
#endif
