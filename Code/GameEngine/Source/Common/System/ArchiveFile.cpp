...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
			// Local filesystem entries are normalized before insertion. Match
			// that representation so a loose file shadows the same archive
			// path instead of causing INI::loadDirectory to parse it twice.
...
const ArchivedFileInfo * ArchiveFile::getArchivedFileInfo(const AsciiString& filename) const
{
	...
	for (Int i = 0; i < count; ++i) {
		const ArchivedFileInfo *info = ArchiveLookupInArchiveTree(&m_rootDirectory, candidates[i]);
		if (info != NULL) {
			return info;
		}
	}
