...
{
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	const char* p = RenegadePathLastSep(in.str());
#else
...
AsciiString GameState::realMapPathToPortableMapPath(const AsciiString& in) const
{
	AsciiString path = in;
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	RenegadeNormalizeAsciiPath(path);
#endif
	AsciiString prefix;
	if (path.startsWithNoCase(getSaveDirectory()))
	{
		prefix = PORTABLE_SAVE;
		prefix.concat(getMapLeafName(path));
	}
	else if (path.startsWithNoCase(TheMapCache->getMapDir()))
	{
		prefix = PORTABLE_MAPS;
		prefix.concat(getMapLeafAndDirName(path));
	}
	else if (path.startsWithNoCase(TheMapCache->getUserMapDir()))
	{
	...
	prefix.toLower();
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/* Portable keys keep '\\' so save files stay compatible with Windows. */
	{
		char buf[_MAX_PATH * 2];
		strncpy(buf, prefix.str(), sizeof(buf) - 1);
		buf[sizeof(buf) - 1] = '\0';
		for (char *p = buf; *p; ++p) {
			if (*p == '/') {
				*p = '\\';
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/*
	 * Portable paths are lowercased for cross-machine keys, but the real path
	 * must keep filesystem casing (e.g. ".../Save/" vs ".../save/"). Lowercasing
	 * the absolute path made extractAndSaveMap fail on Linux and abort loadGame.
	 */
	RenegadeNormalizeAsciiPath(prefix);
#else
	prefix.toLower();
#endif
	return prefix;
