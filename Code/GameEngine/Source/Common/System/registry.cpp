...
Bool GetStringFromRegistry(AsciiString path, AsciiString key, AsciiString& val)
{
	...
	return getStringFromRegistry(HKEY_CURRENT_USER, fullPath, key, val);
}
...
Bool GetUnsignedIntFromRegistry(AsciiString path, AsciiString key, UnsignedInt& val)
{
	...
	if (getUnsignedIntFromRegistry(HKEY_LOCAL_MACHINE, fullPath, key, val))
	{
		return TRUE;
	}

	return getUnsignedIntFromRegistry(HKEY_CURRENT_USER, fullPath, key, val);
}

static Bool readLanguageFromOptionsIni(AsciiString &val)
{
	...
	path.concat("Options.ini");

#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	char normalized[_MAX_PATH * 2];
	strncpy(normalized, path.str(), sizeof(normalized) - 1);
	normalized[sizeof(normalized) - 1] = '\0';
	RenegadeNormalizePathSeparators(normalized);
	path = normalized;
#endif
	...
	while (fgets(buf, sizeof(buf), fp) != NULL) {
		...
		if (key.compareNoCase("Language") == 0) {
			val = rest;
			found = TRUE;
			break;
		}
...
AsciiString GetRegistryLanguage(void)
{
	...
	if (!readLanguageFromOptionsIni(val)) {
