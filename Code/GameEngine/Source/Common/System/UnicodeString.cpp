...
void UnicodeString::ensureUniqueBufferOfSize(int numCharsNeeded, Bool preserveData, const WideChar* strToCopy, const WideChar* strToCat)
{
			...
			m_data->m_numCharsAllocated >= numCharsNeeded)
	{
		...
		if (strToCopy) {
			const size_t copyLen = wcslen(strToCopy) + 1;
			wmemmove(m_data->peek(), strToCopy, copyLen);
		}
		if (strToCat) {
			const size_t baseLen = wcslen(m_data->peek());
			const size_t catLen = wcslen(strToCat) + 1;
			wmemmove(m_data->peek() + baseLen, strToCat, catLen);
		}
		return;
	}
	...
	if (actualBytes < (int)sizeof(UnicodeStringData))
		throw ERROR_OUT_OF_MEMORY;
	...
	if (strToCopy) {
		const size_t copyLen = wcslen(strToCopy) + 1;
		wmemmove(newData->peek(), strToCopy, copyLen);
	}
	if (strToCat) {
		const size_t baseLen = wcslen(newData->peek());
		const size_t catLen = wcslen(strToCat) + 1;
		wmemmove(newData->peek() + baseLen, strToCat, catLen);
	}
...
void UnicodeString::translate(const AsciiString& stringSrc)
{
	...
	clear();
	Int byteLen = stringSrc.getLength();
	if (byteLen <= 0) {
		validate();
		return;
	}
	Int need = MultiByteToWideChar(CP_UTF8, 0, stringSrc.str(), byteLen, NULL, 0);
	if (need <= 0) {
		validate();
		return;
	}
	WideChar *buf = NEW WideChar[need + 1];
	Int wrote = MultiByteToWideChar(CP_UTF8, 0, stringSrc.str(), byteLen, buf, need);
	if (wrote < 0) {
		wrote = 0;
	}
	buf[wrote] = L'\0';
