...
UnicodeString ConfigStringToUnicodeString(AsciiString original)
{
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/*
	 * Old Generals builds stored UTF-16 code units as _XX hex escapes
	 * (e.g. "Лё" -> "_1B_04Q_04"). New builds store UTF-8. Detect legacy
	 * by round-tripping through the original quoted-printable codec.
	 */
	if (original.isEmpty()) {
		return UnicodeString::TheEmptyString;
	}
	UnicodeString legacy = QuotedPrintableToUnicodeString(original);
	if (UnicodeStringToQuotedPrintable(legacy) == original) {
		return legacy;
	}
	UnicodeString utf8;
	utf8.translate(original);
	return utf8;
#else
	return QuotedPrintableToUnicodeString(original);
#endif
}

AsciiString UnicodeStringToConfigString(UnicodeString original)
{
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	AsciiString utf8;
	utf8.translate(original);
	return utf8;
#else
	return UnicodeStringToQuotedPrintable(original);
#endif
}
