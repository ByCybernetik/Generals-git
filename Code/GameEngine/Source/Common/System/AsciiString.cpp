...
void AsciiString::ensureUniqueBufferOfSize(int numCharsNeeded, Bool preserveData, const char* strToCopy, const char* strToCat)
{
			...
			m_data->m_numCharsAllocated >= numCharsNeeded)
	{
		...
		if (strToCopy) {
			const size_t copyLen = strlen(strToCopy) + 1;
			memmove(m_data->peek(), strToCopy, copyLen);
		}
		if (strToCat) {
			const size_t baseLen = strlen(m_data->peek());
			const size_t catLen = strlen(strToCat) + 1;
			memmove(m_data->peek() + baseLen, strToCat, catLen);
		}
		return;
	}
	...
	if (actualBytes < (int)sizeof(AsciiStringData))
		throw ERROR_OUT_OF_MEMORY;
	...
	if (strToCopy) {
		const size_t copyLen = strlen(strToCopy) + 1;
		memmove(newData->peek(), strToCopy, copyLen);
	}
	if (strToCat) {
		const size_t baseLen = strlen(newData->peek());
		const size_t catLen = strlen(strToCat) + 1;
		memmove(newData->peek() + baseLen, strToCat, catLen);
	}
...
void AsciiString::translate(const UnicodeString& stringSrc)
{
	...
	clear();
	Int wideLen = stringSrc.getLength();
	if (wideLen <= 0) {
		validate();
		return;
	}
	Int need = WideCharToMultiByte(
		CP_UTF8, 0, stringSrc.str(), wideLen, NULL, 0, NULL, NULL);
	if (need <= 0) {
		validate();
		return;
	}
	...
	Int wrote = WideCharToMultiByte(
		CP_UTF8, 0, stringSrc.str(), wideLen, buf, need, NULL, NULL);
	if (wrote < 0) {
		wrote = 0;
	}
	buf[wrote] = '\0';
