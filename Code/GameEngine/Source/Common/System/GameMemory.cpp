...
void MemoryPoolObject_DeleteInstance(MemoryPoolObject *obj)
{
	if (obj == NULL) {
		return;
	}
	MemoryPool *pool = obj->getObjectMemoryPool(); // save this, since the dtor will nuke our vtbl
	obj->~MemoryPoolObject();	// it's virtual, so the right one will be called.
	pool->freeBlock((void *)obj);
}

void MemoryPoolObject::deleteInstance()
{
	MemoryPoolObject_DeleteInstance(this);
}
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	void										**m_linuxHeapBlocks;	///< per-slot heap backing (Linux)
#endif
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	Bool linuxContainsUserPointer(const void *pUserData) const;
	Bool linuxContainsBlockHeader(const MemoryPoolSingleBlock *block) const;
#endif
...
{
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	if (m_linuxHeapBlocks != NULL) {
		for (Int i = 0; i < m_totalBlocksInBlob; ++i) {
			if (m_linuxHeapBlocks[i] != NULL) {
				linuxPoolFreeRawBlock(m_linuxHeapBlocks[i]);
			}
		}
		std::free(m_linuxHeapBlocks);
		m_linuxHeapBlocks = NULL;
	}
#endif
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	m_linuxHeapBlocks = static_cast<void **>(std::calloc(static_cast<size_t>(m_totalBlocksInBlob), sizeof(void *)));
	if (m_linuxHeapBlocks == NULL) {
		throw ERROR_OUT_OF_MEMORY;
	}
	for (Int i = 0; i < m_totalBlocksInBlob; ++i) {
		m_linuxHeapBlocks[i] = linuxPoolMallocRawBlock(static_cast<size_t>(rawBlockSize));
	}
	for (Int i = 0; i < m_totalBlocksInBlob; ++i) {
		MemoryPoolSingleBlock *block = static_cast<MemoryPoolSingleBlock *>(m_linuxHeapBlocks[i]);
		MemoryPoolSingleBlock *next = (i + 1 < m_totalBlocksInBlob)
			? static_cast<MemoryPoolSingleBlock *>(m_linuxHeapBlocks[i + 1])
			: NULL;
#ifdef MEMORYPOOL_DEBUG
...
#endif
		block->setNextFreeBlock(next);
#ifdef MEMORYPOOL_DEBUG
		block->debugMarkBlockAsFree();
#endif
	}
	m_firstFreeBlock = static_cast<MemoryPoolSingleBlock *>(m_linuxHeapBlocks[0]);
#else
...
Bool MemoryPoolBlob::debugIsBlockInBlob(void *pBlockPtr)
{
	...
	if (m_linuxHeapBlocks != NULL) {
		for (Int i = m_totalBlocksInBlob - 1; i >= 0; --i) {
			if (static_cast<MemoryPoolSingleBlock *>(m_linuxHeapBlocks[i]) == block) {
				return true;
			}
		}
		return false;
...
//-----------------------------------------------------------------------------
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
Bool MemoryPoolBlob::linuxContainsBlockHeader(const MemoryPoolSingleBlock *block) const
{
	if (!block || !m_owningPool) {
		return false;
	}
	if (m_linuxHeapBlocks != NULL) {
		for (Int i = 0; i < m_totalBlocksInBlob; ++i) {
			if (block == static_cast<MemoryPoolSingleBlock *>(m_linuxHeapBlocks[i])) {
				return true;
			}
		}
		return false;
	...
	const uintptr_t header = reinterpret_cast<uintptr_t>(block);
	const uintptr_t base = reinterpret_cast<uintptr_t>(m_blockData);
	const uintptr_t end = base + static_cast<uintptr_t>(rawBlockSize) * static_cast<uintptr_t>(m_totalBlocksInBlob);
	if (header < base || header >= end) {
		return false;
	}
	return ((header - base) % static_cast<uintptr_t>(rawBlockSize)) == 0;
}

//-----------------------------------------------------------------------------
Bool MemoryPoolBlob::linuxContainsUserPointer(const void *pUserData) const
{
	if (!pUserData || !m_owningPool) {
		return false;
	}
	const Int rawBlockSize = MemoryPoolSingleBlock::calcRawBlockSize(m_owningPool->getAllocationSize());
	const uintptr_t user = reinterpret_cast<uintptr_t>(pUserData);
	uintptr_t expected = sizeof(MemoryPoolSingleBlock);
#ifdef MEMORYPOOL_BOUNDINGWALL
	expected += WALLSIZE;
#endif
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	if (m_linuxHeapBlocks != NULL) {
		for (Int i = 0; i < m_totalBlocksInBlob; ++i) {
			const uintptr_t base = reinterpret_cast<uintptr_t>(m_linuxHeapBlocks[i]);
			const uintptr_t end = base + static_cast<uintptr_t>(rawBlockSize);
			if (user >= base + expected && user < end) {
				return true;
			}
		}
		return false;
	...
	if (!linuxPointerIsPlausibleHeap(m_blockData)) {
		return false;
	}
	const uintptr_t base = reinterpret_cast<uintptr_t>(m_blockData);
	const uintptr_t end = base + static_cast<uintptr_t>(rawBlockSize) * static_cast<uintptr_t>(m_totalBlocksInBlob);
	...
	const uintptr_t offset = user - base;
	const uintptr_t offsetInBlock = offset % static_cast<uintptr_t>(rawBlockSize);
	return offsetInBlock == expected;
}
#endif
...
MemoryPoolBlob* MemoryPool::createBlob(Int allocationCount)
{
	...
	MemoryPoolBlob* blob = new (static_cast<void*>(::sysAllocate(sizeof(MemoryPoolBlob)))) MemoryPoolBlob;	// will throw on failure
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	if (blob != NULL
		&& (!linuxPointerIsPlausibleHeap(blob) || !blob->linuxContainsBlockHeader(block))) {
		blob = linuxFindBlobForUserPointer(pBlockPtr, &block);
	}
#endif
#ifdef MEMORYPOOL_DEBUG
...
//-----------------------------------------------------------------------------
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
Bool MemoryPool::linuxContainsUserPointer(const void *pUserData) const
{
	for (MemoryPoolBlob *blob = m_firstBlob; blob; blob = blob->getNextInList()) {
		if (blob->linuxContainsUserPointer(pUserData)) {
			return true;
		}
	}
	return false;
...
//-----------------------------------------------------------------------------
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
MemoryPoolBlob *MemoryPool::linuxFindBlobForUserPointer(void *pUserData, MemoryPoolSingleBlock **outBlock) const
{
	for (MemoryPoolBlob *blob = m_firstBlob; blob; blob = blob->getNextInList()) {
		if (!blob->linuxContainsUserPointer(pUserData)) {
			continue;
		}
		MemoryPoolSingleBlock *block = MemoryPoolSingleBlock::recoverBlockFromUserData(pUserData);
		if (blob->linuxContainsBlockHeader(block)) {
			if (outBlock) {
				*outBlock = block;
			}
			return blob;
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	if (numSubPools > MAX_DYNAMICMEMORYALLOCATOR_SUBPOOLS) {
		numSubPools = MAX_DYNAMICMEMORYALLOCATOR_SUBPOOLS;
	}
	m_linuxDmaAllocator = new LinuxDmaAllocator();
	m_linuxDmaAllocator->init(numSubPools, pParms);
	m_numPools = 0;
	for (Int i = 0; i < MAX_DYNAMICMEMORYALLOCATOR_SUBPOOLS; i++) {
		m_pools[i] = NULL;
	}
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	if (m_linuxDmaAllocator != NULL) {
		delete m_linuxDmaAllocator;
		m_linuxDmaAllocator = NULL;
	}
	return;
#endif
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	++m_usedBlocksInDma;
	return m_linuxDmaAllocator->allocate(numBytes);
#else
...
//-----------------------------------------------------------------------------
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
Bool DynamicMemoryAllocator::linuxOwnsUserPointer(void *pUserData) const
{
	return m_linuxDmaAllocator != NULL && m_linuxDmaAllocator->owns(pUserData);
}
#endif
...
void DynamicMemoryAllocator::freeBytes(void* pBlockPtr)
{
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	if (m_linuxDmaAllocator != NULL && m_linuxDmaAllocator->owns(pBlockPtr)) {
		ScopedCriticalSection scopedCriticalSection(TheDmaCriticalSection);
		m_linuxDmaAllocator->free(pBlockPtr);
		--m_usedBlocksInDma;
		DEBUG_ASSERTCRASH(m_usedBlocksInDma >= 0, ("negative count for m_usedBlocksInDma"));
		return;
	}
	...
	MemoryPoolSingleBlock *block = MemoryPoolSingleBlock::recoverBlockFromUserData(pBlockPtr);
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	if (block->getOwningBlob() == NULL) {
		Bool rawMatch = false;
		for (MemoryPoolSingleBlock *rb = m_rawBlocks; rb; rb = rb->getNextRawBlock()) {
			if (rb == block && rb->getUserData() == pBlockPtr) {
				rawMatch = true;
				break;
			}
		}
		...
		MemoryPoolBlob *blob = block->getOwningBlob();
		if (blob == NULL
			|| !linuxPointerIsPlausibleHeap(blob)
			|| !blob->linuxContainsBlockHeader(block)
			|| !blob->linuxContainsUserPointer(pBlockPtr)) {
			MemoryPool *resolvedPool = NULL;
			for (Int i = 0; i < m_numPools; i++) {
				if (m_pools[i] == NULL) {
					continue;
				}
				blob = m_pools[i]->linuxFindBlobForUserPointer(pBlockPtr, &block);
				if (blob != NULL) {
					resolvedPool = m_pools[i];
					break;
				}
			}
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	if (m_linuxDmaAllocator != NULL) {
		return m_linuxDmaAllocator->actualAllocationSize(numBytes);
	}
#endif
	MemoryPool *pool = findPoolForSize(numBytes);
...
MemoryPool *MemoryPoolFactory::createMemoryPool(const char *poolName, Int allocationSize, Int initialAllocationCount, Int overflowAllocationCount)
{
	...
	pool = new (static_cast<void*>(::sysAllocate(sizeof(MemoryPool)))) MemoryPool;	// will throw on failure
...
DynamicMemoryAllocator *MemoryPoolFactory::createDynamicMemoryAllocator(Int numSubPools, const PoolInitRec pParms[])
{
	...
	dma = new (static_cast<void*>(::sysAllocate(sizeof(DynamicMemoryAllocator)))) DynamicMemoryAllocator;	// will throw on failure
...
//-----------------------------------------------------------------------------
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
static inline bool linuxOperatorNewUsesMalloc(void)
{
	return !theMainInitFlag;
}

static inline void linuxOperatorDelete(void *p)
{
	if (p == NULL) {
		return;
	}
...
*/
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
__attribute__((constructor(101))) static void linux_disable_glibc_mmap_arenas(void)
{
	linuxInitGameMemoryHeapPolicy();
}
#endif
...
void initMemoryManager()
{
	...
	if (TheMemoryPoolFactory == NULL) 
	{
		...
		TheMemoryPoolFactory = new (static_cast<void*>(::sysAllocate(sizeof(MemoryPoolFactory)))) MemoryPoolFactory;	// will throw on failure
