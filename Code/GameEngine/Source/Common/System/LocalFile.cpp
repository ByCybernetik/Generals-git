...
static void LinuxNormalizePathSeparators(const char *in, char *out, size_t out_cap)
{
	size_t j = 0;
...
Bool LocalFile::open( const Char *filename, Int access )
{
	...
	LinuxNormalizePathSeparators(filename, normalizedPath, sizeof(normalizedPath));
	const Char *openPath = normalizedPath;
#else
	...
	*m++ = 0;
	...
	m_file = fopen(openPath, mode);
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	if (m_file == NULL && (m_access & WRITE) == 0)
	{
		char resolved[_MAX_PATH];
		if (Platform_Resolve_File_Path_Case(openPath, resolved, sizeof(resolved)))
		{
			m_file = fopen(resolved, mode);
		}
	}
#endif
	if (m_file == NULL)
	...
	m_handle = _open( openPath, flags , _S_IREAD | _S_IWRITE);
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	if (m_handle == -1 && (m_access & WRITE) == 0)
	{
		char resolved[_MAX_PATH];
		if (Platform_Resolve_File_Path_Case(openPath, resolved, sizeof(resolved)))
		{
			m_handle = _open(resolved, flags, _S_IREAD | _S_IWRITE);
		}
	}
#endif
