...
void GameLODManager::init(void)
{
	...
	/*
	 * Linux port: missing/broken Options.ini used to fall back to UNKNOWN/LOW.
	 * Default to HIGH so shadows, textures, and effects match desktop expectations.
	 */
	if (userSetDetail == STATIC_GAME_LOD_UNKNOWN) {
	...
	/* Signed wrap of >2GB RAM yields a negative m_numRAM; that still means
	 * far more than 256MB, so treat it as a pass. */
	if (m_numRAM < 0 || (Real)(m_numRAM)/(Real)(256*1024*1024) >= PROFILE_ERROR_LIMIT)
	...
	if (TheWritableGlobalData) {
		...
		/* Cloud map + lightmap re-enabled — SPIR-V terrain modulates them in-pass. */
		TheWritableGlobalData->m_disableFog = TRUE;
		fprintf(stderr, "shadow_diag: volumes/decals forced OFF\n");
		fprintf(stderr, "fog_diag: distance fog forced OFF; fog-of-war follows GlobalData\n");
		fprintf(stderr, "terrain_diag: CloudMap/LightMap enabled (SPIR-V)\n");
		fflush(stderr);
	}
#endif
...
#if defined(RENEGADE_VULKAN)
		/* Force-disable all shadows until stencil/decal paths are fixed. */
		TheWritableGlobalData->m_useShadowVolumes = FALSE;
		TheWritableGlobalData->m_useShadowDecals = FALSE;
#endif
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/* cpuFreq==0 means detection failed, not a 1990s CPU. Do not kill the
	 * main-menu 3D shell map on a false "really low MHz" reading. */
