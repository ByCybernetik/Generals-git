...
// Stable storage for map paths referenced by listbox item data (str() must outlive selection).
static std::list<AsciiString> g_mapListboxPathStorage;

static const char *storeMapListboxPath(const AsciiString &path)
{
	g_mapListboxPathStorage.push_back(path);
	return g_mapListboxPathStorage.back().str();
}
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	RenegadeNormalizeAsciiPath(tmp);
#endif
	return tmp;
}
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	RenegadeNormalizeAsciiPath(filepath);
	{
		static int map_cache_path_log = 0;
		if (map_cache_path_log < 4) {
			++map_cache_path_log;
		}
	}
#endif
...
Bool MapCache::loadUserMaps()
{
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		RenegadeNormalizeAsciiPath(fname);
#endif
		File *fp = TheFileSystem->openFile(fname.str(), File::READ);
		if (fp)
		{
	...
	while (iter != filenameList.end()) {
		...
		tempfilename = (*iter);
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		RenegadeNormalizeAsciiPath(tempfilename);
		AsciiString lowerPath = tempfilename;
		lowerPath.toLower();
#else
		...
		const char *s = lowerPath.reverseFind('\\');
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		if (!s) {
			s = lowerPath.reverseFind('/');
		}
#endif
		if (!s)
		{
		...
		else
		{
			...
			if (!skipMap)
			{
				Bool endingOk = lowerPath.endsWithNoCase(endingStr.str());
				Bool infoOk = FALSE;
				Bool added = FALSE;
				if (!endingOk)
				{
					DEBUG_CRASH(("Found map '%s' in wrong spot (%s)", fname.str(), lowerPath.str()));
				}
				else
				{
					if (TheFileSystem->getFileInfo(tempfilename, &fileInfo)) {
						infoOk = TRUE;
						m_seen[lowerPath] = TRUE;
						added = addMap(mapDir, tempfilename, &fileInfo, TheGlobalData->m_buildMapCache);
						parsedAMap |= added;
					} else {
						DEBUG_CRASH(("Could not get file info for map %s", tempfilename.str()));
					}
				}
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
				{
					static int map_file_log = 0;
					if (map_file_log < 12) {
						++map_file_log;
					}
				}
...
Bool MapCache::addMap( AsciiString dirName, AsciiString fname, FileInfo *fileInfo, Bool isOfficial)
{
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	RenegadeNormalizeAsciiPath(dirName);
	RenegadeNormalizeAsciiPath(fname);
#endif
	...
	if (it != end())
	{
		...
		{
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
			it->second.m_realFilePath = fname;
#endif
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		{
			const char *leafSep = RenegadePathLastSep(fname.str());
			tempdisplayname = (leafSep != NULL) ? (leafSep + 1) : fname;
		}
#else
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		RenegadeNormalizeAsciiPath(stringFileName);
#endif
		for (Int i=0; i<4; ++i)
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	RenegadeNormalizeAsciiPath(mapDir);
#endif
	mapDir.toLower();
...
typedef MapDisplayToFileNameList::iterator MapDisplayToFileNameListIter;
		...
		while (tempit != tempCache.end())
		{
			...
			*/
			...
			if (it->first.startsWithNoCase(mapDir.str()) && isMultiplayer == it->second.m_isMultiplayer && !it->second.m_displayName.isEmpty())
			{
				...
				// Store path in stable storage; cache keys may be reallocated during updateCache().
				GadgetListBoxSetItemData( listbox, (void *)storeMapListboxPath(it->first), index );
	...
	{
		void *itemData = NULL;
		Int numEntries = GadgetListBoxGetNumEntries(listbox);
		if (mapsListed > 0 && selectionIndex >= 0 && selectionIndex < numEntries)
			itemData = GadgetListBoxGetItemData(listbox, selectionIndex);
	}
#endif
	...
	{
		Int numEntries = GadgetListBoxGetNumEntries(listbox);
		if (selectionIndex < 0 || selectionIndex >= numEntries)
			selectionIndex = 0;
		if (!GadgetListBoxGetItemData(listbox, selectionIndex))
		{
			selectionIndex = -1;
			for (Int row = 0; row < numEntries; ++row)
			{
				if (GadgetListBoxGetItemData(listbox, row))
				{
					selectionIndex = row;
					break;
				}
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	{
		static int populate_maps_log = 0;
		if (populate_maps_log < 16) {
			++populate_maps_log;
		}
	}
#endif
...
const MapMetaData *MapCache::findMap(AsciiString mapName)
{
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/*
	 * Portable/real path helpers use '/', but official MapCache.ini keys from
	 * maps.big still use Windows '\'. Normalize lookup and fall back to '\' keys.
	 */
	RenegadeNormalizeAsciiPath(mapName);
#endif
	mapName.toLower();
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	{
		...
		strncpy(buf, mapName.str(), sizeof(buf) - 1);
		buf[sizeof(buf) - 1] = '\0';
		for (char *p = buf; *p; ++p) {
			if (*p == '/') {
				*p = '\\';
...
Image *getMapPreviewImage( AsciiString mapName )
{
	if(!TheGlobalData)
		return NULL;
	AsciiString ioMapPath = mapName;
	if (TheMapCache)
	{
		const MapMetaData *md = TheMapCache->findMap(mapName);
		if (md && !md->m_realFilePath.isEmpty())
			ioMapPath = md->m_realFilePath;
	}
	DEBUG_LOG(("%s Map Name \n", ioMapPath.str()));
	AsciiString tgaName = ioMapPath;
	AsciiString name;
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	{
		const char *leafSep = RenegadePathLastSep(tgaName.str());
		filename = (leafSep != NULL) ? (leafSep + 1) : tgaName;
	}
#else
