...
//-----------------------------------------------------------------------------
void LookAtTranslator::applyScroll( Real dtScale )
{
	if (!TheInGameUI || !TheTacticalView || !TheGlobalData)
		return;

	if (dtScale < 0.05f)
		dtScale = 0.05f;
	else if (dtScale > 3.0f)
		dtScale = 3.0f;

	Coord2D offset = {0, 0};

	/* Forced stop from script / UI. */
...
GameMessageDisposition LookAtTranslator::translateGameMessage(const GameMessage *msg)
{
	...
	switch (t)
	{
		...
		case GameMessage::MSG_RAW_MOUSE_POSITION:
		{
			...
			const Bool allowScreenEdgeScroll = TRUE;
#else
			const Bool allowScreenEdgeScroll = !TheGlobalData->m_windowed;
#endif
			if (allowScreenEdgeScroll)
			{
				if (m_isScrolling)
				{
		...
		{
			/*
			 * On Linux, camera scroll is applied from LookAtTranslator::applyScroll()
			 * every visual frame (dt-scaled). Elsewhere keep one scroll per logic tick.
			 */
#if !defined(GENERALS_LINUX) && !defined(RENEGADE_LINUX)
			applyScroll(1.0f);
#endif
