...
UnicodeString SkirmishPreferences::getUserName(void)
{
	...
	ret = ConfigStringToUnicodeString(it->second);
	ret.trim();
	if (ret.isEmpty())
	{
		IPEnumeration IPs;
...
Bool SkirmishPreferences::write(void)
{
	...
	(*this)["UserName"] = UnicodeStringToConfigString(TheSkirmishGameInfo->getConstSlot(0)->getName());
...
void reallyDoStart( void )
{
	...
	/* Cache keys are lowercased; on Linux open the real-cased path for user maps. */
	if (md && !md->m_realFilePath.isEmpty())
		TheWritableGlobalData->m_mapName = md->m_realFilePath;
	else
...
static void startPressed(void)
{
	...
	const MapMetaData *mdCheck = TheMapCache->findMap(TheSkirmishGameInfo->getMap());
	if (mdCheck == NULL)
	{
		buttonPushed = FALSE;
...
void SkirmishGameOptionsMenuInit( WindowLayout *layout, void *userData )
{
  ...
  TheSkirmishGameInfo->setSlot(0,gSlot);
	...
	{
		AsciiString savedSlots = prefs.getSlotList();
		if (!savedSlots.isEmpty()) {
			ParseAsciiStringToGameInfo(TheSkirmishGameInfo, savedSlots);
		}
		/* SlotList round-trips the name via UTF-8; keep UserName as source of truth
		 * for the local player so a bad/legacy SlotList cannot truncate Cyrillic. */
		UnicodeString userName = prefs.getUserName();
		if (!userName.isEmpty() && TheSkirmishGameInfo->getSlot(0)) {
			TheSkirmishGameInfo->getSlot(0)->setName(userName);
		}
	}
...
//	NameKeyType sliderGameSpeedID = TheNameKeyGenerator->nameToKey( AsciiString( "SkirmishGameOptionsMenu.wnd:SliderGameSpeed" ) );
  ...
  TheShell->showShellMap(TRUE);
	/* Do not restart Shell if it is already playing — that rewinds the track.
	 * Only start it when nothing is playing (e.g. after credits / load). */
	if (TheAudio)
	{
		Bool playing = TheAudio->isMusicPlaying();
		if (!playing)
		{
			AudioEventRTS shellMusic(AsciiString("Shell"));
			TheAudio->addAudioEvent(&shellMusic);
		}
	}
