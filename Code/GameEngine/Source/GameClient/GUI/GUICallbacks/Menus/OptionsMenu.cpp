...
AsciiString OptionPreferences::getLanguage(void)
{
	OptionPreferences::const_iterator it = find("Language");
...
void OptionPreferences::setLanguage(AsciiString lang)
{
	lang.trim();
	if (lang.isEmpty()) {
		lang = "english";
...
static Bool parseYesNoPreference(const AsciiString &val, Bool defaultValue)
{
	if (val.isEmpty()) {
		return defaultValue;
	}
	if (stricmp(val.str(), "yes") == 0 || stricmp(val.str(), "true") == 0 || strcmp(val.str(), "1") == 0) {
		return TRUE;
	}
	if (stricmp(val.str(), "no") == 0 || stricmp(val.str(), "false") == 0 || strcmp(val.str(), "0") == 0) {
		return FALSE;
	}
	return defaultValue;
}
...
Bool OptionPreferences::getWindowed(void)
{
	...
	if (it != end()) {
		return parseYesNoPreference(it->second, TheGlobalData->m_windowed);
	}

	it = find("FullScreen");
	if (it != end()) {
		Bool fullscreen = parseYesNoPreference(it->second, !TheGlobalData->m_windowed);
		return !fullscreen;
	}
...
{
	(*this)["Windowed"] = windowed ? AsciiString("yes") : AsciiString("no");
}
...
//	// speaker type
  ...
  if( index >= 0 && TheGlobalData->m_antiAliasBoxValue != index )
  {
	...
	pref->setWindowed(TheGlobalData->m_windowed);
}
...
//	checkBoxLowTextureDetail      = TheWindowManager->winGetWindowFromId( NULL, checkBoxLowTextureDetailID );
  ...
  enum AliasingMode
  {
	...
	AsciiString temp;
	Int i;
	for (i=0; i < NUM_ALIASING_MODES; ++i)
	{
