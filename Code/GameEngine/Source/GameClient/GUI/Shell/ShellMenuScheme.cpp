...
static Bool schemeImagesSame( const Image *a, const Image *b )
{
	if (a == b) {
		return TRUE;
	}
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	std::list<ShellMenuSchemeImage *> remaining(m_imageList.begin(), m_imageList.end());

	while (!remaining.empty()) {
		ShellMenuSchemeImage *part = remaining.front();
		remaining.pop_front();
		if (part == NULL || part->m_image == NULL) {
			continue;
		}

		Int startX = part->m_position.x;
		Int startY = part->m_position.y;
		Int endX = startX + part->m_size.x;
		Int endY = startY + part->m_size.y;
		Image *img = part->m_image;
		Bool extended = TRUE;

		while (extended) {
			extended = FALSE;
			ShellMenuSchemeImageListIt nextIt = remaining.begin();
			while (nextIt != remaining.end()) {
				ShellMenuSchemeImage *next = *nextIt;
				if (next != NULL && schemeImagesSame(next->m_image, img) &&
						next->m_position.y == startY &&
						next->m_size.y == part->m_size.y &&
						next->m_position.x == endX) {
					endX = next->m_position.x + next->m_size.x;
					nextIt = remaining.erase(nextIt);
					extended = TRUE;
				} else {
					++nextIt;
				}
		...
		TheDisplay->drawImage(img, startX, startY, endX, endY);
	}
