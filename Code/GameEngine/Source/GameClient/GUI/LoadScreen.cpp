...
void LoadScreen::update( Int percent )
{
	...
	/* GameEngine::update() is suspended during map load; Miles streams refill
	 * themselves, but SDL music needs AudioManager::update() to keep feeding. */
	if (TheAudio)
		TheAudio->update();
...
*/
	...
	AsciiString movieLabel = mission ? mission->m_movieLabel : AsciiString::TheEmptyString;
	m_videoStream = TheVideoPlayer->open( movieLabel );
	if ( m_videoStream == NULL )
	{
		DEBUG_LOG(("SinglePlayerLoadScreen: failed to open movie '%s'\n", movieLabel.str()));
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		fprintf(stderr, "SinglePlayerLoadScreen: failed to open movie '%s'\n",
			movieLabel.isEmpty() ? "(empty)" : movieLabel.str());
#endif
		m_percent->winHide(TRUE);
		return;
	}
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	fprintf(stderr,
		"SinglePlayerLoadScreen: playing '%s' %dx%d frames=%d\n",
		movieLabel.str(),
		m_videoStream->width(),
	...
	if(TheGameLODManager && TheGameLODManager->didMemPass())
	{
		...
		while (m_videoStream->frameIndex() < m_videoStream->frameCount() - 1 )
		{
			...
			/*
			 * Briefing voice starts mid-movie (frame 255). Without Audio::update
			 * the speech stream never refills and underruns for the rest of the
			 * blocking video loop — Miles AIL_quick_* needed no per-frame service.
			 */
			if (TheAudio)
				TheAudio->update();
