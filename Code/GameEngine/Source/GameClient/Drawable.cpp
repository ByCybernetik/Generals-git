				...
				: Thing( thingTemplate )
{
  ...
  m_fadeMode = FADING_NONE;
	...
	m_hasVisualInterpPrev = false;
	m_visualInterpPrevBatchId = 0;
	m_visualInterpPrevPos.x = m_visualInterpPrevPos.y = m_visualInterpPrevPos.z = 0.0f;
	m_physicsXformLastWallMs = 0;
	m_airSmoothPos.x = m_airSmoothPos.y = m_airSmoothPos.z = 0.0f;
	m_airSmoothValid = false;
	m_airSmoothLastMs = 0;
	m_airSmoothYaw = 0.0f;
	m_airSmoothYawValid = false;
#endif
...
Bool Drawable::getShouldAnimate( Bool considerPower ) const
{
	...
	/*
	 * Death / explosion poses must keep animating even when the object is
	 * disabled (e.g. DISABLED_HELD during SlowDeath sink). Otherwise the HLOD
	 * stays in ANIM_MODE_MANUAL and the corpse looks like a frozen skinned mesh.
	 */
	if (getModelConditionFlags().test(MODELCONDITION_DYING)
		|| getModelConditionFlags().test(MODELCONDITION_EXPLODED_FLAILING)
		|| getModelConditionFlags().test(MODELCONDITION_EXPLODED_BOUNCING)
		|| (obj && obj->isEffectivelyDead()))
	{
		return TRUE;
	}
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/*
	 * fix16: micro-jitter remained with dVis~0 but dPitch/s up to ~3.3 on free helis —
	 * client hover lean was still integrating every visual frame. Skip for airborne;
	 * bank/lean from logic pose is enough at 60 FPS without the chassis spring fight.
	 */
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/*
	 * Hover chassis: wall-clock dt (~30 Hz rate at any visual FPS). Cam-lock close-up
	 * stability is handled by skipping translation interp on the locked drawable.
	 */
	{
		const DWORD nowMs = timeGetTime();
		Real dtMs = (m_physicsXformLastWallMs != 0) ? (Real)(nowMs - m_physicsXformLastWallMs)
			: (1000.0f / (Real)LOGICFRAMES_PER_SECOND);
		m_physicsXformLastWallMs = nowMs;
		if (dtMs < 1.0f)
			dtMs = 1.0f;
		else if (dtMs > 100.0f)
			dtMs = 100.0f;
		dtScale = dtMs * ((Real)LOGICFRAMES_PER_SECOND / 1000.0f);
		if (dtScale < 0.05f)
			dtScale = 0.05f;
		else if (dtScale > 2.5f)
			dtScale = 2.5f;
	}
#endif
...
//const Coord3D *pos = getPosition();
	...
	m_locoInfo->m_pitchRate += ((-PITCH_STIFFNESS * m_locoInfo->m_pitch) + (-PITCH_DAMPING * m_locoInfo->m_pitchRate)) * dtScale;
	m_locoInfo->m_rollRate += ((-ROLL_STIFFNESS * m_locoInfo->m_roll) + (-ROLL_DAMPING * m_locoInfo->m_rollRate)) * dtScale;

	m_locoInfo->m_pitch += m_locoInfo->m_pitchRate * UNIFORM_AXIAL_DAMPING * dtScale;
	m_locoInfo->m_roll += m_locoInfo->m_rollRate   * UNIFORM_AXIAL_DAMPING * dtScale;

	// process chassis acceleration dynamics - damp back towards zero

	m_locoInfo->m_accelerationPitchRate += ((-PITCH_STIFFNESS * (m_locoInfo->m_accelerationPitch)) + (-PITCH_DAMPING * m_locoInfo->m_accelerationPitchRate)) * dtScale;
	m_locoInfo->m_accelerationPitch += m_locoInfo->m_accelerationPitchRate * dtScale;

	m_locoInfo->m_accelerationRollRate += ((-ROLL_STIFFNESS * (m_locoInfo->m_accelerationRoll)) + (-ROLL_DAMPING * m_locoInfo->m_accelerationRollRate)) * dtScale;
	m_locoInfo->m_accelerationRoll += m_locoInfo->m_accelerationRollRate * dtScale;
	...
	if (physics->isMotive()) 
  {
		if (Z_VEL_PITCH_COEFF != 0.0f)
		{
			...
			if (fabs(vel->z) > TINY_DZ)
			{
				...
				m_locoInfo->m_pitch -= Z_VEL_PITCH_COEFF * pitch * dtScale;
			}
		}

		// cause the chassis to pitch & roll in reaction to current speed
		Real forwardVel = dir->x * vel->x + dir->y * vel->y;
		m_locoInfo->m_pitch += -(FORWARD_VEL_COEFF * forwardVel) * dtScale;

		Real lateralVel = -dir->y * vel->x + dir->x * vel->y;
		m_locoInfo->m_roll += -(LATERAL_VEL_COEFF * lateralVel) * dtScale;

		// cause the chassis to pitch & roll in reaction to acceleration/deceleration
		Real forwardAccel = dir->x * accel->x + dir->y * accel->y;
		m_locoInfo->m_accelerationPitchRate += -(FORWARD_ACCEL_COEFF * forwardAccel) * dtScale;

		Real lateralAccel = -dir->y * accel->x + dir->x * accel->y;
		m_locoInfo->m_accelerationRollRate += -(LATERAL_ACCEL_COEFF * lateralAccel) * dtScale;
	}
...
void Drawable::draw( View *view )
{
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/*
	 * Translation-only smoothing between logic ticks. Full Matrix3D::Lerp shook units
	 * (quaternion slerp). Only lerp if prev was captured in the latest logic batch —
	 * otherwise stopped units rubber-banded on stale prev.
	 */
	{
		Coord3D drawPos;
		getVisualDrawPosition(drawPos);
		Object *obj = getObject();
		const Bool airFree = obj && obj->isUsingAirborneLocomotor()
			&& !(TheTacticalView && TheTacticalView->getCameraLock() == obj->getID());
		if (airFree && m_airSmoothYawValid)
		{
			/* Rebuild facing from smoothed yaw; physics xform still adds hover lean. */
			transformMtx.Make_Identity();
			transformMtx.Rotate_Z(m_airSmoothYaw);
			transformMtx.Set_Translation(Vector3(drawPos.x, drawPos.y, drawPos.z));
		}
		else
		{
			transformMtx.Set_Translation(Vector3(drawPos.x, drawPos.y, drawPos.z));
	...
	/* applyPhysicsXform already done above on Linux when building drawPos */
#if !(defined(GENERALS_LINUX) || defined(RENEGADE_LINUX))
	applyPhysicsXform(&transformMtx);
#endif
...
//-------------------------------------------------------------------------------------------------
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
void Drawable::setVisualInterpPrevPos( const Coord3D& prev )
{
	/* First move in this logic batch becomes prev; later catch-up ticks keep that window. */
	if (TheGameClient)
	{
		const UnsignedInt batch = TheGameClient->getVisualInterpBatchId();
		if (m_hasVisualInterpPrev && m_visualInterpPrevBatchId == batch)
			return;
		m_visualInterpPrevBatchId = batch;
	}
	m_visualInterpPrevPos = prev;
	m_hasVisualInterpPrev = true;
}

void Drawable::getVisualDrawPosition( Coord3D& out ) const
{
	const Coord3D* cur = getPosition();
	out = *cur;
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/*
	 * Airborne: alpha-lerp fights logic yaw snaps (fix13 fast dVis~2–5). Use wall-clock
	 * exponential chase toward logic pos instead — no alpha reset, damps hover noise.
	 * Camera-lock: snap to logic (fix12 cam dVis=0 was stable in frame center).
	 */
	if (m_object && m_object->isUsingAirborneLocomotor())
	{
		if (TheTacticalView && TheTacticalView->getCameraLock() == m_object->getID())
		{
			m_airSmoothValid = false;
			m_airSmoothYawValid = false;
			return;
		...
		const DWORD nowMs = timeGetTime();
		Real dtMs = (m_airSmoothLastMs != 0) ? (Real)(nowMs - m_airSmoothLastMs)
			: (1000.0f / (Real)LOGICFRAMES_PER_SECOND);
		m_airSmoothLastMs = nowMs;
		if (dtMs < 1.0f)
			dtMs = 1.0f;
		else if (dtMs > 100.0f)
			dtMs = 100.0f;

		const Real logicMs = 1000.0f / (Real)LOGICFRAMES_PER_SECOND;
		Real a = 1.0f - powf(0.01f, dtMs / logicMs);
		if (a < 0.0f) a = 0.0f;
		if (a > 1.0f) a = 1.0f;
		...
		{
			m_airSmoothPos = *cur;
			m_airSmoothValid = true;
		}
		else
		{
			const Real dx = cur->x - m_airSmoothPos.x;
			const Real dy = cur->y - m_airSmoothPos.y;
			const Real dz = cur->z - m_airSmoothPos.z;
			const Real distSq = dx * dx + dy * dy + dz * dz;
			Real maxDist = 40.0f;
			if (TheGameEngine)
			{
				const Int mapHz = TheGameEngine->getMapLogicHz();
				if (mapHz > 0 && mapHz < LOGICFRAMES_PER_SECOND)
					maxDist *= (Real)LOGICFRAMES_PER_SECOND / (Real)mapHz;
			}
			if (distSq > maxDist * maxDist || distSq < 1.0f)
			{
				/* teleport OR settle-snap (fix15 still had chase shimmer ~0.7) */
				m_airSmoothPos = *cur;
			}
			else
			{
				m_airSmoothPos.x += dx * a;
				m_airSmoothPos.y += dy * a;
				m_airSmoothPos.z += dz * a;
			}
		}

		/* fix15: residual jerk correlated with logic yaw jumps (Δorient up to 0.38/tick). */
		{
			Real targetYaw = m_object->getOrientation();
			if (!m_airSmoothYawValid)
			{
				m_airSmoothYaw = targetYaw;
				m_airSmoothYawValid = true;
			}
			else
			{
				Real diff = targetYaw - m_airSmoothYaw;
				while (diff > PI)
					diff -= PI * 2.0f;
				while (diff < -PI)
					diff += PI * 2.0f;
				m_airSmoothYaw += diff * a;
			}
		}
...
#endif
	if (!(m_hasVisualInterpPrev && TheGameClient
		&& m_visualInterpPrevBatchId == TheGameClient->getVisualInterpBatchId()))
		return;
	Real alpha = TheGameClient->getVisualInterpAlpha();
	if (alpha < 0.0f)
		alpha = 0.0f;
	else if (alpha > 1.0f)
		alpha = 1.0f;
	if (alpha >= 0.999f)
		return;
	const Real dx = cur->x - m_visualInterpPrevPos.x;
	const Real dy = cur->y - m_visualInterpPrevPos.y;
	const Real dz = cur->z - m_visualInterpPrevPos.z;
	const Real distSq = dx * dx + dy * dy + dz * dz;
	Real maxDist = 40.0f;
	if (TheGameEngine)
	{
	...
	if (distSq > maxDist * maxDist)
		return;
	out.x = m_visualInterpPrevPos.x + dx * alpha;
	out.y = m_visualInterpPrevPos.y + dy * alpha;
	out.z = m_visualInterpPrevPos.z + dz * alpha;
}
#endif
