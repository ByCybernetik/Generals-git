...
void Display::update( void )
{
	if ( m_videoStream && m_videoBuffer )
	{
		...
		static const int kMaxVideoCatchUpFrames = 8;
		int catch_up = 0;
		while (catch_up < kMaxVideoCatchUpFrames && m_videoStream->isFrameReady())
#else
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
			catch_up++;
			/* stopMovie() nulls the stream; do not touch it after end-of-movie. */
			if (m_videoStream == NULL ||
