...
GameClient::GameClient()
{
	...
	m_visualInterpAlpha = 1.0f;
	m_visualInterpBatchId = 0;
#endif
...
void GameClient::update( void )
{
	...
	{
		if( playSizzle && !TheGlobalData->m_breakTheMovie)
		{
			TheWritableGlobalData->m_allowExitOutOfMovies = TRUE;
			if(TheGameLODManager && TheGameLODManager->didMemPass())
	...
	{
		// Decode Bink frames before drawing (DRAW used to run first → blank/white video).
		TheDisplay->UPDATE();
		TheDisplay->DRAW();
		return;
	}
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		/*
		 * Draw happens in renderVisualFrame() so visual FPS can exceed logic rate.
		 * Intro/logo path above still draws immediately (returns early).
		 */
#else
		TheDisplay->DRAW();
#endif
...
/** -----------------------------------------------------------------------------------------------
 * Visual-only frame between logic ticks: fresh input for camera scroll, dt-scaled scroll, draw.
 * Does not advance drawables/particles or create MSG_FRAME_TICK.
 */
void GameClient::renderVisualFrame( Real dtMs )
{
#if !(defined(GENERALS_LINUX) || defined(RENEGADE_LINUX))
	(void)dtMs;
	return;
	...
	if (TheGlobalData->m_playIntro || TheGlobalData->m_afterIntro)
	{
		/* Intro path already drew inside update(). */
		return;
	}
	...
	/*
	 * Do NOT call Mouse::update() here — it drains the OS event buffer. Clicks would be
	 * overwritten before GameClient::update() runs createStreamMessages on the logic tick,
	 * which breaks UI at visual FPS > 30.
	 * Peek cursor only (no button drain) so RMB/edge scroll tracks smoothly between ticks.
	 */
	if (TheLookAtTranslator)
	{
		...
		Real frameMs = MSEC_PER_LOGICFRAME_REAL;
		if (frameMs < 1.0f)
			frameMs = 33.0f;
		TheLookAtTranslator->applyScroll(dtMs / frameMs);
	}
