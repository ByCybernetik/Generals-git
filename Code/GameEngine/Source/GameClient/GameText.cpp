	...
	m_failed()
{
	...
	m_failed.translate(AsciiString("***FATAL*** String Manager failed to initilaize properly"));
...
void GameTextManager::translateCopy( WideChar *outbuf, Char *inbuf )
{
	...
	while( *inbuf != '\0' )
	{
		if( slash == TRUE )
		{
			...
			switch( *inbuf )
			{
				...
				default:
					...
					*outbuf++ = (WideChar)(unsigned char)(*inbuf);
#else
					*outbuf++ = *inbuf & 0x00FF;
#endif
					break;
		...
		{
			/* Decode UTF-8 so .str files can carry Cyrillic etc. as UTF-8. */
			...
			if (p[0] < 0x80u) {
				...
				cp = ((uint32_t)(p[0] & 0x0Fu) << 12) |
					...
					(uint32_t)(p[2] & 0x3Fu);
				n = 3;
			} else if ((p[0] & 0xF8u) == 0xF0u && p[1] != 0 && p[2] != 0 && p[3] != 0) {
			...
			if (cp <= 0xFFFFu) {
				...
				*outbuf++ = (WideChar)(0xD800u + (v >> 10));
				*outbuf++ = (WideChar)(0xDC00u + (v & 0x3FFu));
			} else {
				*outbuf++ = (WideChar)0xFFFDu;
			}
			inbuf += n;
		}
#else
...
Bool GameTextManager::parseCSF( const Char *filename )
{
	...
	if ( header.id != CSF_ID || header.num_labels <= 0 || header.num_labels != m_textCount )
	{
		file->close();
		return FALSE;
	}
	...
	while( file->read ( &id, sizeof (id)) == sizeof ( id) )
	{
		...
		if ( listCount >= m_textCount )
		{
			goto quit;
		}

		if ( file->read ( &num_strings, sizeof ( Int ) ) != sizeof ( Int ) )
		{
			goto quit;
		}
		...
		{
		 	if ( file->read ( &id, sizeof ( Int ) ) != sizeof ( Int ) )
			{
				goto quit;
			}
			...
			if ( len > 0 )
			{
				const Int byteCount = len * static_cast<Int>(sizeof(WideChar));
				if ( byteCount / static_cast<Int>(sizeof(WideChar)) != len )
				{
					goto quit;
				}
				if ( file->read ( m_tbuffer, byteCount ) != byteCount )
				{
					goto quit;
				}
			}
