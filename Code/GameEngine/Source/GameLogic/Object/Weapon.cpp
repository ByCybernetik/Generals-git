...
#include "GameLogic/DamageNameTables.h"
...
//		m_name.str(),sourceObj->getTemplate()->getName().str(),victimObj?victimObj->getTemplate()->getName().str():"NULL"));
	...
	else	// must be a projectile
	{
		...
		DumbProjectileBehavior* dumbProj = NULL;
		MissileAIUpdate* missileAI = NULL;
		for (BehaviorModule** u = projectile->getBehaviorModules(); *u; ++u)
		{
			dumbProj = DumbProjectileBehavior::asDumbProjectile(*u);
			if (dumbProj)
				break;
			missileAI = MissileAIUpdate::asMissileAI(*u);
			if (missileAI)
				break;
		...
		{
			VeterancyLevel v = sourceObj->getVeterancyLevel();
			dumbProj->projectileLaunchAtObjectOrPosition(victimObj, &projectileDestination, sourceObj, wslot, specificBarrelToUse, this, m_projectileExhausts[v]);
		}
		else if (missileAI)
		{
			VeterancyLevel v = sourceObj->getVeterancyLevel();
			missileAI->projectileLaunchAtObjectOrPosition(victimObj, &projectileDestination, sourceObj, wslot, specificBarrelToUse, this, m_projectileExhausts[v]);
		}
		else if (pui)
		{
			VeterancyLevel v = sourceObj->getVeterancyLevel();
...
)
{
	...
	/* Ignore launcher before setPosition — otherwise the shell can collide with
	 * the firer on the same call and detonate at the muzzle. */
