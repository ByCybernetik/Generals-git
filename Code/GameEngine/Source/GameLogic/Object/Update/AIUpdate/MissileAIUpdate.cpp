...
//-------------------------------------------------------------------------------------------------
ProjectileUpdateInterface* MissileAIUpdate::getProjectileUpdateInterface()
{
	return static_cast<ProjectileUpdateInterface*>(this);
}

//-------------------------------------------------------------------------------------------------
MissileAIUpdate* MissileAIUpdate::asMissileAI(BehaviorModule* mod)
{
	if (mod == NULL || mod->getUpdate() == NULL)
		return NULL;
	/* Module name is assigned by ModuleFactory as the class name string. */
	static const NameKeyType key = NAMEKEY("MissileAIUpdate");
	if (mod->getModuleNameKey() != key)
		return NULL;
	/* UpdateModule is the first base of AIUpdateInterface / MissileAIUpdate. */
	return static_cast<MissileAIUpdate*>(static_cast<UpdateModule*>(mod->getUpdate()));
}
	...
	if (other != NULL)
	{
		...
		if (d->m_garrisonHitKillCount > 0)
		{
			...
			if( contain && contain->getContainCount() > 0 && contain->isGarrisonable() && !contain->isImmuneToClearBuildingAttacks() )
			{
				...
				if (items)
				{
					/* See DumbProjectileBehavior — *it != NULL is UB on libstdc++ end(). */
