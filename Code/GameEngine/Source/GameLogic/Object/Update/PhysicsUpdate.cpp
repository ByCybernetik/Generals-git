...
static ProjectileUpdateInterface* getPui(Object* obj)
{
	...
	{
		/* Prefer primary-base path for MI projectile modules (see DumbProjectileBehavior). */
		if (DumbProjectileBehavior* dumb = DumbProjectileBehavior::asDumbProjectile(*u))
			return dumb;
		if (MissileAIUpdate* missile = MissileAIUpdate::asMissileAI(*u))
			return missile;
