...
//-------------------------------------------------------------------------------------------------
ProjectileUpdateInterface* DumbProjectileBehavior::getProjectileUpdateInterface()
{
	return static_cast<ProjectileUpdateInterface*>(this);
}

//-------------------------------------------------------------------------------------------------
DumbProjectileBehavior* DumbProjectileBehavior::asDumbProjectile(BehaviorModule* mod)
{
	if (mod == NULL || mod->getUpdate() == NULL)
		return NULL;
	/* Module name is assigned by ModuleFactory as the class name string. */
	static const NameKeyType key = NAMEKEY("DumbProjectileBehavior");
	if (mod->getModuleNameKey() != key)
		return NULL;
	/* UpdateModule is the first base of DumbProjectileBehavior. */
	return static_cast<DumbProjectileBehavior*>(static_cast<UpdateModule*>(mod->getUpdate()));
}
...
Bool DumbProjectileBehavior::projectileHandleCollision( Object *other )
{
		...
		if (other != NULL)
	{
		...
		/* Not launched / missing warhead template — ignore collision. */
		...
		if (d->m_garrisonHitKillCount > 0)
		{
			...
			if( contain && contain->getContainCount() > 0 && contain->isGarrisonable() && !contain->isImmuneToClearBuildingAttacks() )
			{
				...
				{
					/*
					 * Must use it != end(), not *it != NULL. MSVC-era lists often had a
					 * null-valued sentinel; libstdc++ end() is past-the-end — dereferencing
					 * it SEGV'd (thingToKill=0x8) when flash-bangs clear garrisons.
					 */
					for (ContainedItemsList::const_iterator it = items->begin();
						it != items->end() && numKilled < d->m_garrisonHitKillCount; )
					{
						Object* thingToKill = *it++;
						if (thingToKill != NULL && !thingToKill->isEffectivelyDead() && thingToKill->isKindOfMulti(d->m_garrisonHitKillKindof, d->m_garrisonHitKillKindofNot))
						{
...
UpdateSleepTime DumbProjectileBehavior::update()
{
	...
	/* Not launched yet: empty path must not mean "detonate now". */
	if (m_detonationWeaponTmpl == NULL && m_launcherID == INVALID_ID)
