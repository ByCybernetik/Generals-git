...
// ------------------------------------------------------------------------------------------------
/** Keep SDL/FFmpeg music fed while GameEngine::update() is suspended during map load. */
// ------------------------------------------------------------------------------------------------
static void pumpAudioForLoad(Bool forcePrefill = FALSE)
{
	if (!TheAudio) {
		return;
	}

	static UnsignedInt s_lastPumpMs = 0;
	const UnsignedInt now = timeGetTime();
	if (!forcePrefill) {
		if (now - s_lastPumpMs < 40u) {
			return;
		}
		TheAudio->update();
		s_lastPumpMs = now;
	...
	/* Prefill stream queue before a long blocking section (terrain/pathfind). */
	for (Int i = 0; i < 20; ++i) {
		TheAudio->update();
	}
	s_lastPumpMs = timeGetTime();
}
...
void GameLogic::startNewGame( Bool saveGame )
{
	...
	// Prefill load music — TerrainLogic::loadMap can block for seconds with no progress ticks.
	pumpAudioForLoad(TRUE);
...
*/
	...
	MapObject *pMapObj;
	UnsignedInt bridgePumpTimer = timeGetTime();
	for (pMapObj = MapObject::getFirstMapObject(); pMapObj; pMapObj = pMapObj->getNext()) 
	{
		...
		if (timeGetTime() > bridgePumpTimer + 100) {
			pumpAudioForLoad();
			bridgePumpTimer = timeGetTime();
		}
	...
	if( saveGame == FALSE )
	{
		...
		for (pMapObj = MapObject::getFirstMapObject(); pMapObj; pMapObj = pMapObj->getNext()) 
		{
			...
			const Int nowMs = timeGetTime();
			/* Audio-only pump often enough to keep SDL music fed; full UI tick stays rarer. */
			if (nowMs > audioTimer + 80) {
				pumpAudioForLoad();
				audioTimer = nowMs;
			}
			if(nowMs > timer + 500)
	...
	while(!isProgressComplete())
	{
		...
		testTimeOut();
		/* GameEngine::update is suspended; SDL streams need AudioManager::update. */
		if (TheAudio)
			TheAudio->update();
		Sleep(100);
	}
	...
	if( saveGame == FALSE )
	{
		/* Fade load/battle music during the visual fade — but never on the
		 * shell map: MainMenu/scripts own that track, and StopTheMusicFade
		 * here left the 3D menu map stuck/black. */
		if (m_gameMode != GAME_SHELL && TheAudio)
			TheAudio->removeAudioEvent(AHSV_StopTheMusicFade);

		TheTransitionHandler->setGroup("FadeWholeScreen");
		/* SCREENFADE is ~30 frames; cap so a stuck non-fire-once group cannot
		 * block shell/battle map entry forever. */
		Int fadeWaitFrames = 0;
		const Int fadeWaitLimit = 90;
		while(!TheTransitionHandler->isFinished() && fadeWaitFrames < fadeWaitLimit)
		{
		...
		if (!TheTransitionHandler->isFinished())
			TheTransitionHandler->remove("FadeWholeScreen", TRUE);
	}
...
#if 1
	const Int sz = (Int)m_sleepyUpdates.size();
	if (i < 0 || i >= sz)
	{
		return i;
	}

	UpdateModulePtr *pBegin = m_sleepyUpdates.data();
	UpdateModulePtr *pI = pBegin + i;

	// our children are i*2 and i*2+1
  Int child = ((i+1)<<1)-1;
	UpdateModulePtr *pChild = pBegin + child;
	UpdateModulePtr *pSZ = pBegin + sz;	// one-past-end sentinel (not valid for operator[])
