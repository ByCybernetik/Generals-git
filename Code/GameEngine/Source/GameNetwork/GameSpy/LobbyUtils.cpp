...
struct GameSortStruct
{
	bool operator()(const GameSpyStagingRoom *g1, const GameSpyStagingRoom *g2) const
	{
		...
		if (sortBuddies)
		{
			Bool g1HasBuddies = (theBuddyGames->find(const_cast<GameSpyStagingRoom *>(g1)) != theBuddyGames->end());
			Bool g2HasBuddies = (theBuddyGames->find(const_cast<GameSpyStagingRoom *>(g2)) != theBuddyGames->end());
