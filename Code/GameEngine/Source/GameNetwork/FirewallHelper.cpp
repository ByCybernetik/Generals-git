...
FirewallHelperClass::FirewallHelperClass(void)
{
	...
	for (Int j = 0; j < MAX_NUM_MANGLERS; j++)
	{
		m_manglers[j] = 0;
	}
...
//	SpareSocketStruct *spareSocket = NULL;
	...
	if (msg == NULL) {
		for (Int j = 0; j < MAX_SPARE_SOCKETS; ++j) {
			if ((m_messages[j].length != 0) && (m_messages[j].data.PacketID == packetID)) {
				msg = &(m_messages[j]);
				msg->length = 0;
			}
		}
	...
	if (m_currentTry < 3) {
		...
		*/
		...
		for (Int j=0 ; j<NUM_TEST_PORTS ; j++) {
			if (m_mangledPorts[j] == 0) {
				sendToManglerFromPort(m_manglers[0], m_sparePorts[j], m_packetID+j);
			}
		}
