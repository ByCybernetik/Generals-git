...
{
	/* Same Win32 5-bit shift mask as Damage/Death flags (LEVEL_REGULAR == 0). */
	const UnsignedInt shift = ((UnsignedInt)(dt - 1)) & 31u;
	return (flags & (1u << shift)) != 0;
}

inline VeterancyLevelFlags setVeterancyLevelFlag(VeterancyLevelFlags flags, VeterancyLevel dt)
{
	const UnsignedInt shift = ((UnsignedInt)(dt - 1)) & 31u;
	return (flags | (1u << shift));
}
