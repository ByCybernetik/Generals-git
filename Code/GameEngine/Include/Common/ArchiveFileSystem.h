...
#define ARCHIVE_LOOKUP_MAX_CANDIDATES 8

// BIG archives (e.g. Textures.big) often store flat filenames while the game
// looks up Art/Textures/name.dds. Build alternate lookup paths and walk trees.
int ArchiveBuildLookupPathCandidates(const AsciiString& filename, AsciiString *candidates, int maxCandidates);
Bool ArchiveLookupInGlobalTree(const ArchivedDirectoryInfo *root, const AsciiString& path, AsciiString *archiveFilenameOut);
const ArchivedFileInfo *ArchiveLookupInArchiveTree(const DetailedArchivedDirectoryInfo *root, const AsciiString& path);
...
protected:
	...
	ArchiveFile *findArchiveFile(const AsciiString& archiveFilename) const;
