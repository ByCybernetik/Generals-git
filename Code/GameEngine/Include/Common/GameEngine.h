...
public:
	...
	/** Map SET_FPS_LIMIT: logic Hz for cinematic authenticity (20/25). 0 = default 30. Does not change visual FPS. */
	void setMapLogicHz( Int hz ) { m_mapLogicHz = (hz > 0) ? hz : 0; }
	Int getMapLogicHz( void ) const { return m_mapLogicHz; }
#endif
...
protected:
	...
	Int m_maxFPS;																									///< Maximum frames per second allowed (visual on Linux)
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	Int m_mapLogicHz;																							///< Map cinematic logic rate; 0 = LOGICFRAMES_PER_SECOND
#endif
