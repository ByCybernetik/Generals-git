...
public:
	...
	Bool m_disableLighting;			///< disable dynamic lighting (flat-shaded view)
	Bool m_disableFog;						///< disable distance fog
	Bool m_renderDebugDisableShroud;		///< hide shroud layer (debug / command)
	Bool m_renderDebugDisableFogOfWar;	///< hide fog-of-war style (debug / command)
...
inline Bool GlobalData_Shroud_Render_Enabled(const GlobalData *gd)
{
	if (gd == NULL) {
		return TRUE;
	}
#if defined(_DEBUG) || defined(_INTERNAL)
	return gd->m_shroudOn;
#else
	return !gd->m_renderDebugDisableShroud;
#endif
}

inline Bool GlobalData_Fog_Of_War_Render_Enabled(const GlobalData *gd)
{
	if (gd == NULL) {
		return TRUE;
	}
#if defined(_DEBUG) || defined(_INTERNAL)
	return gd->m_fogOfWarOn;
#else
	return !gd->m_renderDebugDisableFogOfWar;
#endif
}
