...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/// return true iff pUserData is a user-data pointer inside this pool's blobs
	Bool linuxContainsUserPointer(const void *pUserData) const;
	MemoryPoolBlob *linuxFindBlobForUserPointer(void *pUserData, MemoryPoolSingleBlock **outBlock) const;
#endif
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	LinuxDmaAllocator					*m_linuxDmaAllocator;	///< libc-heap DMA backend (replaces sub-pools on Linux)
#endif
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	/// return true iff pUserData was allocated by this DMA (subpool or raw block)
	Bool linuxOwnsUserPointer(void *pUserData) const;
#endif
...
// ----------------------------------------------------------------------------
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
#define MEMORY_POOL_DTOR_PREFIX public:
#else
#define MEMORY_POOL_DTOR_PREFIX protected:
#endif

#define MEMORY_POOL_GLUE_WITHOUT_GCMP(ARGCLASS) \
	MEMORY_POOL_DTOR_PREFIX \
	virtual ~ARGCLASS(); \
public: \
...
protected: 
	...
	friend void MemoryPoolObject_DeleteInstance(MemoryPoolObject *obj);
...
extern void userMemoryAdjustPoolSize(const char *poolName, Int& initialAllocationCount, Int& overflowAllocationCount);
...
#if defined(_WIN32) && !defined(GENERALS_LINUX) && !defined(RENEGADE_LINUX)
	...
	// additional overloads for 'placement new' (MSVC only; conflicts with libstdc++ on Linux)
#ifdef _WIN32
