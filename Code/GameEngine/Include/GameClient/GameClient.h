...
public:
	...
	/** Visual-only frame: input, dt-scaled camera scroll, draw. No logic/drawables/particles. */
	virtual void renderVisualFrame( Real dtMs );
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	void setVisualInterpAlpha( Real alpha ) { m_visualInterpAlpha = alpha; }
	Real getVisualInterpAlpha( void ) const { return m_visualInterpAlpha; }
	void beginVisualInterpLogicBatch( void ) { ++m_visualInterpBatchId; }
	UnsignedInt getVisualInterpBatchId( void ) const { return m_visualInterpBatchId; }
#endif
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	Real m_visualInterpAlpha;																		///< 0=prev logic pos, 1=current
	UnsignedInt m_visualInterpBatchId;													///< logic-batch id for prev capture
#endif
