...
public:
	...
	/** Render-only: remember pose before a logic move (translation lerp). */
	void setVisualInterpPrevPos( const Coord3D& prev );
	/** Position used for drawing this frame (logic pos, or translation-lerped). */
	void getVisualDrawPosition( Coord3D& out ) const;
#endif
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
	Coord3D m_visualInterpPrevPos;			///< position before last logic move (draw only)
	Bool m_hasVisualInterpPrev;
	UnsignedInt m_visualInterpPrevBatchId;	///< GameClient batch when prev was captured
	UnsignedInt m_physicsXformLastWallMs;	///< wall clock for hover chassis dt scale
	mutable Coord3D m_airSmoothPos;			///< exp-smooth draw pos for free airborne
	mutable Bool m_airSmoothValid;
	mutable UnsignedInt m_airSmoothLastMs;
	mutable Real m_airSmoothYaw;			///< exp-smooth facing for free airborne
	mutable Bool m_airSmoothYawValid;
#endif
