...
private:
	...
	/*
	 * Store heap FXList* — not FXList by value. std::unordered_map (hash_map on
	 * Linux) invalidates references/pointers into values on rehash; Object/Weapon
	 * modules and nested FXListAtBonePos keep const FXList* for the whole session.
	 * MSVC's old hash_map was often node-stable; unordered_map is not.
	 */
