...
public:
	...
	/*
	 * Must not be inline: with UpdateModule + ProjectileUpdateInterface MI,
	 * GCC needs the out-of-line return so PUI* is correctly adjusted. Inline
	 * "return this" has been observed to hand Weapon/Physics a primary-base
	 * pointer labeled as PUI*, so virtual calls use the wrong this and never
	 * write m_detonationWeaponTmpl / flight path (tank shells die instantly).
	 */
	virtual ProjectileUpdateInterface* getProjectileUpdateInterface();

	/** Downcast via primary UpdateModule base (offset 0) — safe without RTTI. */
	static DumbProjectileBehavior* asDumbProjectile(BehaviorModule* mod);
