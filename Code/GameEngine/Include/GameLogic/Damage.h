...
/*
 * Flag bit index: original EA used (dt - 1). On Win32/MSVC, unsigned long is
 * 32-bit and shift counts are masked to 5 bits, so dt==0 (DAMAGE/DEATH/LEVEL
 * first enum = 0) maps to bit 31. On Linux LP64, 1UL<<(0-1) becomes 1UL<<63,
 * which never intersects a 32-bit flags word — DieMux always rejects REGULAR
 * units and DEATH_NORMAL. Match Win32: mask shift to 5 bits, use 32-bit 1u.
 */
inline UnsignedInt rtsFlagBitIndex(Int dt)
{
	return ((UnsignedInt)(dt - 1)) & 31u;
}

inline Bool getDamageTypeFlag(DamageTypeFlags flags, DamageType dt)
{
	return (flags & (1u << rtsFlagBitIndex((Int)dt))) != 0;
}

inline DamageTypeFlags setDamageTypeFlag(DamageTypeFlags flags, DamageType dt)
{
	return (flags | (1u << rtsFlagBitIndex((Int)dt)));
}
...
#if defined(DEFINE_DAMAGE_NAMES) || defined(DEFINE_DEATH_NAMES)
