			...
			case WM_KEYDOWN:
			{
				...
				switch( key )
				{
					...
					case VK_ESCAPE:
					{
						...
						/* SDL delivers ESC here; do not tear down the main loop. */
						return 0;
#else
						PostQuitMessage( 0 );
						break;
...
static Bool initializeAppWindows( HINSTANCE hInstance, Int nCmdShow, Bool runWindowed )
{
	...
	ApplicationHWnd = Platform_Get_Main_HWnd();
	Platform_Apply_Window_Display_Mode(runWindowed, DEFAULT_XRESOLUTION, DEFAULT_YRESOLUTION);
	gInitializing = false;
	return ApplicationHWnd != NULL;
#else
	DWORD windowStyle;
  ...
  HWND hWnd = CreateWindow( TEXT("Game Window"),
	...
	return true;  // success
#endif /* !GENERALS_LINUX && !RENEGADE_LINUX */
                      ...
                      LPSTR lpCmdLine, Int nCmdShow )
{
	...
	try {
		...
		if (Linux_FindGameInstallDirectory(gameDir, sizeof(gameDir))) {
		...
		*/
		...
		if (argc>2 && strcmp(argv[1],"-DX")==0) {  
			...
			for (i=2; i<argc; i++) {
				...
				unsigned int line;
				uintptr_t addr;
				GetFunctionDetails(UINTPTR_TO_POINTER(pc), name, file, &line, &addr);
				DEBUG_LOG(("0x%zx - %s, %s, line %d address 0x%zx\n", pc, name, file, line, addr));
			}
...
#if defined(GENERALS_LINUX) || defined(RENEGADE_LINUX)
		Platform_Set_Window_Message_Handler(
			(Platform_Window_Message_Handler)WndProc);
#endif
